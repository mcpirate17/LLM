...
from research.scientist.leaderboard_scoring import build_score_kwargs, compute_composite_v11
...
def main():
    ...
    rows = conn.execute("SELECT * FROM leaderboard ORDER BY composite_score DESC LIMIT 50").fetchall()

    rescored = []
    for row in rows:
        d = dict(row)
        kwargs = build_score_kwargs(conn, None, d['result_id'], d, d.get('is_reference', 0))
    ...
    print(f"{'Rank':<5} | {'New Score':<10} | {'Old Score':<10} | {'Tokenizer':<10} | {'Description'}")
    print("-" * 80)
    for i, item in enumerate(rescored[:15]):
        print(f"{i+1:<5} | {item['new_score']:<10.2f} | {item['old_score']:<10.2f} | {str(item['tok']):<10} | {item['desc']}")
