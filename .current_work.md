...
- Compared against the packed-IR boundary notes above. This does not revive
  ...
  for this workload.
- Retained:
  - explicit runtime-first packed-IR validation paths use the existing C runtime
    analyzer before Python-extension paths. The public `analyze_ir_natively()`
    compatibility order still honors `aria_core` monkeypatch/backend contracts.
...
- Median movement versus
  `runtime_hotpaths_current_final_selector_and_template_batch.json`:
  - graph fingerprint: small `1.506 -> 1.189 ms`, medium `5.760 -> 4.723 ms`,
    large `24.092 -> 20.394 ms`;
  - novelty scores: small `2.248 -> 2.078 ms`, medium `8.377 -> 7.622 ms`,
    large `33.202 -> 31.693 ms`;
  - routing telemetry: small `1.427 -> 1.349 ms`, medium `7.786 -> 7.351 ms`,
    large `29.512 -> 27.957 ms`;
  - graph generation stayed noisy: small `15.369 -> 15.081 ms`, medium
    `62.427 -> 65.902 ms`, large `293.802 -> 280.531 ms`.
- Validation:
  - focused graph/native/routing suite: `97 passed in 10.05s`.
- Blunt assessment:
  - the useful C move here is targeted runtime selection: use the existing C
    packed-IR analyzer in validation paths before Python-extension paths that
    require Python object materialization.
...
- Retained only the slots totals + existing batched torch histogram reduction:
  - isolated `workload_routing_telemetry(128)` moved
    `7.731 ms -> 7.240 ms` median (`6.4% faster`).
- Rejected:
  - per-histogram `add_` accumulation (`10.329 ms`, slower);
  - local `rt.get` micro-change (`7.468 ms`, slower than retained slots path).
- Fixed surfaced synthesis regression:
  - restored public `analyze_ir_natively()` aria-core-first behavior while
    keeping explicit runtime-first validation in `analyze_ir_runtime_first()`.
- Validation:
