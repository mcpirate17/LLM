...
CMD=$(cat | python3 -c "import sys,json; ti=json.load(sys.stdin).get('tool_input',{}); print(ti.get('command') or ti.get('cmd') or '')" 2>/dev/null || true)
...
if [ -x "$HOOK_DIR/_bash_impact.py" ]; then
    python3 "$HOOK_DIR/_bash_impact.py" <<EOF || true
