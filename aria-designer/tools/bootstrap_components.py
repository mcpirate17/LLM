...
+ kernel_fallback.py for each component under aria-designer/components/.
...
def build_primitive_manifest(op) -> Dict[str, Any]:
    ...
    category = OP_CATEGORY_MAP.get(op.category.value, "math")
...
def write_component(manifest: Dict[str, Any], dry_run: bool = False) -> Path:
    """Write a component folder with manifest.yaml + kernel_fallback.py."""
    comp_dir = COMPONENTS_DIR / manifest["category"] / manifest["id"]
    manifest_path = comp_dir / "manifest.yaml"
    kernel_path = comp_dir / "kernel_fallback.py"
