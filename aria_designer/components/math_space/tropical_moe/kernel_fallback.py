...
class ComponentHandler:
    ...
    def forward(self, inputs, config):
        ...
        if self._trop_weight is None or self._trop_weight.shape[0] != D:
            self._trop_weight = nn.Parameter(torch.randn(D, 2, device=x.device, dtype=x.dtype) * 0.02)
        ...
        trop_scores = (x.unsqueeze(-1) + self._trop_weight).min(dim=-2).values  # (B, S, 2)
        gate_weights = F.softmax(-trop_scores, dim=-1)  # negate: shorter distance = higher weight
