...
try:
    ...
    aria_core = None  # type: ignore[assignment]
    HAS_ARIA_CORE = False
...
class NativeGraphExecutor(nn.Module):
    ...
    def __init__(
        self, graph: ComputationGraph, compiled_layer: Optional[CompiledLayer] = None
    ):
        super().__init__()
        self.graph = graph
        ...
        for nid in self.topo:
            ...
            shape = (
                node.output_shape.batch,
                node.output_shape.seq,
                node.output_shape.dim,
            )
            actual_shape = [
                1 if d == "B" else (16 if d == "S" else int(d)) for d in shape
            ]
            self._tensor_pool[idx] = torch.zeros(
                actual_shape, dtype=torch.float32, device="cpu"
            ).contiguous()
            current_idx += 1
            ...
            if nid in self.compiled_layer.ops:
                ...
                if op_name == "rmsnorm":
                    ...
                    self._tensor_pool[current_idx] = torch.zeros(
                        node.output_shape.dim, dtype=torch.float32, device="cpu"
                    ).contiguous()
                    current_idx += 1
                elif op_name in (
                    "linear",
                    ...
                    self.param_map[(nid, "weight")] = current_idx
                    self._tensor_pool[current_idx] = torch.zeros(
                        target.weight.shape, dtype=torch.float32, device="cpu"
                    ).contiguous()
                    current_idx += 1
                    self.param_map[(nid, "bias")] = current_idx
                    self._tensor_pool[current_idx] = torch.zeros(
                        target.weight.shape[0], dtype=torch.float32, device="cpu"
                    ).contiguous()
                    current_idx += 1
            ...
            nodes_to_bake.append(
                {
                    ...
                    "params": [node.config.get("eps", 1e-6)]
                    if "eps" in node.config
                    else [],
                }
            )
    ...
    def _sync_parameters(self):
        ...
        for nid in self.topo:
            node = self.graph.nodes[nid]
            if nid not in self.compiled_layer.ops:
                continue
            ...
            if op_name == "rmsnorm":
                ...
                if hasattr(target, "weight"):
                    ...
                    if hasattr(target, "bias") and target.bias is not None:
                        ...
                        tb = torch.zeros(
                            out_dim, device="cpu", dtype=torch.float32
                        ).contiguous()
