...
function DesignerApp() {
  ...
  const handleResizeKeyDown = useCallback((e) => {
    ...
    if (e.key === 'ArrowLeft') { e.preventDefault(); setRightPanelWidth((w) => Math.max(250, Math.min(900, w + step))); return }
    if (e.key === 'ArrowRight') { e.preventDefault(); setRightPanelWidth((w) => Math.max(250, Math.min(900, w - step))); return }
    if (e.key === 'Home') { e.preventDefault(); setRightPanelWidth(250); return }
    if (e.key === 'End') { e.preventDefault(); setRightPanelWidth(900) }
  }, [])
  ...
  // Autosave (debounced to avoid serializing on every keystroke/drag)
  useEffect(() => {
    if (embeddedMode) {
  ...
  // Keep refs for embedded message handler (direct assignment, no useEffect needed)
  ...
  }, [importResultIntoCanvas, postToParent, workflowMeta, loadWorkflowJsonRef])
