...
function DesignerApp() {
  ...
  }, [handleValidate, handlePreview, handleSave, setNodes])
  ...
  }, [nodes, workflowMeta, loadWorkflowJson, pushToast])

  const displayNodes = useMemo(() => {
    const ghostNodes = ariaSuggestions.map((s, idx) => {
      // Position ghosts in a staggered way if no specific target
      // In a real scenario, the API might suggest a 'target_node_id' to place it near
      const lastNode = nodes[nodes.length - 1]
      const baseX = lastNode ? lastNode.position.x + 250 : 500
      const baseY = lastNode ? lastNode.position.y : 200

      return {
        id: `ghost_${s.id || idx}`,
        type: 'ghost',
        position: { x: baseX, y: baseY + (idx * 160) },
        data: {
          label: s.component?.name || 'Suggestion',
          category: s.component?.category || 'math',
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const lineage = data.fingerprint_changed && data.parent_fingerprint
        ? ` from ${String(data.parent_fingerprint).slice(0, 8)}...`
        : ''
      ...
      setSaveState({
        phase: 'saved',
        message: `Saved v${data.version}${data.fingerprint ? ` · fp ${String(data.fingerprint).slice(0, 12)}...` : ''}`,
        version: Number(data.version) || null,
        fingerprint: data.fingerprint || null,
        at: Date.now(),
      })
      setStatusMsg(`Save complete: v${data.version}${fp}${lineage}${promo}`)
    } catch (err) {
            ...
            try {
              ...
              if (eventType === 'run_id') {
                ...
                const doneMsg = succeeded
                  ? `Deep Run complete (${(payload.total_time_ms / 1000).toFixed(1)}s${scoreText}${baselineLossText})`
  ...
  const handleAskAriaSubmit = useCallback(async (promptText) => {
    ...
    if (lower.includes('split pipeline') || lower.includes('parallel branch') || lower.includes('split branch')) componentType = 'structural/split2'
    if (lower.includes('routing') || lower.includes('top-k') || lower.includes('topk') || lower.includes('early-exit')) componentType = 'routing/mod_topk'
    if (lower.includes('compression') || lower.includes('low-rank') || lower.includes('low rank') || lower.includes('bottleneck')) componentType = 'math_space/low_rank_proj'
  ...
  }, [nodes, edges, workflowMeta, ariaSuggestions, evalState, proposalQuery])
      ...
      <main className="canvas-wrap">
          ...
          <header className="topbar">
            ...
            <div className="actions">
              <input
                ...
                style={{ display: 'none' }}
              />
            <div className="toolbar-group-title">Execution</div>
            ...
            <div className="toolbar-group-title">I/O</div>
            <div className="toolbar-group files">
              <button
                type="button"
                onClick={handleSave}
                title="Save workflow"
                className={`${saveState.phase === 'saving' ? 'busy active' : ''} ${saveState.phase === 'saved' ? 'save-ok' : ''} ${saveState.phase === 'failed' ? 'save-fail' : ''}`}
              ...
              <button type="button" onClick={handleExportJson} title="Export workflow JSON">Export</button>
              <button type="button" onClick={handleExportPython} title="Export Python module">Export Py</button>
              <button type="button" onClick={() => importInputRef.current?.click()} title="Import workflow JSON">Import JSON</button>
              <button type="button" className="primary" onClick={() => setShowImportDialog(true)} title="Import from AI Scientist">Import Research</button>
            </div>
            <div className="toolbar-group-title">Graph Edit</div>
            <div className="toolbar-group library">
              ...
              <button type="button" onClick={handleReloadComponents} title="Reload component library from disk">Reload</button>
              <button type="button" onClick={undoGraph} disabled={!historyUi.canUndo} title="Undo last graph edit (Ctrl/Cmd+Z)">Undo</button>
              <button type="button" onClick={redoGraph} disabled={!historyUi.canRedo} title="Redo graph edit (Ctrl/Cmd+Shift+Z)">Redo</button>
              <button 
                type="button"
              ...
              <button type="button" onClick={handleAlignHorizontal} title="Align selected nodes horizontally (or all if none selected)">Align H</button>
              <button type="button" onClick={handleAlignVertical} title="Align selected nodes vertically (or all if none selected)">Align V</button>
              <button type="button" onClick={handleDistributeHorizontal} title="Distribute selected nodes horizontally with even spacing">Dist H</button>
              <button type="button" onClick={handleDistributeVertical} title="Distribute selected nodes vertically with even spacing">Dist V</button>
              <button type="button" onClick={handleTidySelection} title="Tidy only selected nodes: snap and remove overlaps">Tidy Sel</button>
              <button type="button" onClick={handleClearCanvas} style={{color: '#ff5050'}} title="Clear all nodes and edges from the canvas">Clear</button>
            </div>
            ...
            <div className="toolbar-group ai">
              <button 
                type="button" 
                className={hardwareView ? 'active primary' : ''} 
                onClick={() => setHardwareView(!hardwareView)} 
                title="Hardware View: Highlight nodes with native C/C++ kernel coverage"
              >
                Hardware View
              </button>
              <button 
                type="button" 
                className={heatmapView ? 'active primary' : ''} 
                onClick={() => setHeatmapView(!heatmapView)} 
                title="Heatmap View: Shade nodes by relative FLOPs/Bottleneck intensity"
              >
                Heatmap
              </button>
              <button type="button" className="primary" onClick={() => setShowAskAriaModal(true)}>Ask Aria</button>
              <button type="button" onClick={() => setShowShortcuts(true)} title="Keyboard shortcuts (? to toggle)">
                Shortcuts
              </button>
            </div>
          </div>
        ...
        >
          <ReactFlow
            ...
            onNodeClick={(_, node) => {
              if (node.type === 'ghost') {
                handleGhostClick(node.data.suggestion)
              } else {
      ...
      {!embeddedMode && (
        <aside className="panel right">
          ...
          <div className="panel-tabs">
            ...
            >
              Proposals {scopedProposals.length > 0 ? `(${scopedProposals.length})` : ''}
            </button>
            <button
              type="button"
