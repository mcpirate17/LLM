...
function Palette({ components, onDragStart, onBlockedDrag, constraints = {} }) {
  ...
  const handleDragStart = (e, comp) => {
    ...
    if (constraint?.compatible === false) {
      e.preventDefault()
      if (typeof onBlockedDrag === 'function') {
        const reason = Array.isArray(constraint.reasons) && constraint.reasons.length > 0
          ? constraint.reasons[0]
          : 'Incompatible with current graph context'
