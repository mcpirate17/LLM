...
def propose_patch(patch: AriaPatchProposalModel) -> Dict[str, Any]:
    ...
    now = _utc_now()
    db.save_proposal(proposal_id=proposal_id, workflow_id=patch.workflow_id,
                     patch_json=json.dumps(patch.model_dump()), rationale=patch.rationale, created_at=now)
...
def apply_patch(req: ApplyPatchRequest) -> Dict[str, Any]:
    ...
    if HAS_BRIDGE:
        ...
        if not validation_info.get("valid", False):
            raise HTTPException(status_code=422, detail=f"Patched workflow invalid: {validation_info.get('error', 'unknown error')}")
    ...
    new_version = db.save_workflow(
        ...
        graph_json=json.dumps(patched_workflow), author=f"aria (approved by {req.approved_by})",
        parent_id=f"{workflow_id}@v{wf_row.get('version', 0)}", created_at=now, updated_at=now)
    db.resolve_proposal(req.proposal_id, "applied", req.approved_by, now)
    return {"applied": True, "proposal_id": req.proposal_id, "approved_by": req.approved_by,
            "workflow_id": workflow_id, "new_version": new_version, "ops_applied": len(ops),
            "validation": validation_info, "old_fingerprint": old_fingerprint,
            "new_fingerprint": new_fingerprint, "patched_workflow": patched_workflow}
...
def reject_patch(req: ApplyPatchRequest) -> Dict[str, Any]:
    ...
    return {"rejected": True, "proposal_id": req.proposal_id, "rejected_by": req.approved_by}
...
def record_outcome(req: RecordOutcomeRequest) -> Dict[str, Any]:
    ...
    try:
        db.record_suggestion_outcome(suggestion_id=req.suggestion_id, outcome=req.outcome,
            timestamp=_utc_now(), fingerprint=req.fingerprint, intent=req.intent,
            details=req.details, session_id=req.session_id)
        return {"success": True}
    except Exception as e:
...
def _infer_component_from_prompt(prompt: str, fallback_suggestions: List[Dict[str, Any]]) -> Optional[str]:
    ...
    for c in approved:
        ...
        if name and name != cid: comp_lookup[name] = comp_type
...
def _resolve_node_token(token: str, nodes: List[Dict[str, Any]]) -> Optional[str]:
    ...
    if t in by_id: return by_id[t]
    for n in nodes:
        cid = str(n.get("component_type", "")).lower().split("/")[-1]
        if t == cid or t in cid: return str(n.get("id"))
    return None

def _insertion_hint_payload(workflow: Dict[str, Any], component_type: str | None) -> Dict[str, str | None]:
...
def _build_edge_maps(edges: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], Dict[str, List[Any]]]:
...
def _append_replace_ops(
    ...
    for src_raw, dst_raw in re.findall(r"replace\s+([-\w/]+)\s+with\s+([-\w/ ]+)", lower):
        ...
        if node_id and dst_type:
            ops.append({"op": "replace_node", "node_id": node_id, "payload": {"component_type": dst_type}})


def _append_remove_ops(ops: List[Dict[str, Any]], lower: str, nodes: List[Dict[str, Any]]) -> None:
...
def _append_insert_front_ops(
    ...
    ops.append({"op": "rewire", "payload": {"action": "remove", "source": input_node["id"], "target": first_edge["target"]}})
    ops.append({
        "op": "add_node",
        "payload": {
            "id": node_id,
            ...
            "edges": [
                {"source": input_node["id"], "source_port": "out", "target": node_id, "target_port": "in"},
                {"source": node_id, "source_port": "out", "target": first_edge.get("target", ""), "target_port": first_edge.get("target_port", "in")},
            ],
        },
    })
...
def _append_relative_insert_ops(
    ...
    for add_raw, position, target_raw in re.findall(r"(?:add|insert)\s+([-\w/ ]+?)\s+(before|after)\s+([-\w/]+)", lower):
        ...
        if position == "before":
            ...
            if incoming:
                ...
                ops.append({"op": "rewire", "payload": {"action": "remove", "source": incoming["source"], "target": target_id}})
            payload["edges"].append({
                "source": node_id,
                "source_port": "out",
                "target": target_id,
                "target_port": incoming.get("target_port", "in") if incoming else "in",
            })
        else:
            for outgoing in outgoing_edges.get(target_id, []):
                ...
                ops.append({"op": "rewire", "payload": {"action": "remove", "source": target_id, "target": outgoing["target"]}})
            payload["edges"].append({"source": target_id, "source_port": "out", "target": node_id, "target_port": "in"})
        ops.append({"op": "add_node", "payload": payload})


def _append_connect_ops(ops: List[Dict[str, Any]], lower: str, nodes: List[Dict[str, Any]]) -> None:
    for source_raw, target_raw in re.findall(r"connect\s+([-\w/]+)\s+to\s+([-\w/]+)", lower):
        source_id = _resolve_node_token(source_raw, nodes)
        target_id = _resolve_node_token(target_raw, nodes)
        if source_id and target_id:
            ops.append({"op": "rewire", "payload": {"action": "add", "source": source_id, "source_port": "out", "target": target_id, "target_port": "in"}})


def _append_param_mutation_ops(ops: List[Dict[str, Any]], lower: str, nodes: List[Dict[str, Any]]) -> None:
    for key_raw, node_raw, value_raw in re.findall(r"set\s+([\w]+)\s+of\s+([-\w/]+)\s+to\s+([-\w.]+)", lower):
        node_id = _resolve_node_token(node_raw, nodes)
        if node_id:
            ops.append({"op": "mutate_param", "node_id": node_id, "payload": {key_raw: _coerce_prompt_value(value_raw)}})


def _append_optimize_ops(ops: List[Dict[str, Any]], lower: str, nodes: List[Dict[str, Any]]) -> None:
    if "optimize" not in lower or not any(token in lower for token in ("data", "control", "workflow")):
        return
    for node in nodes:
        component_type = str(node.get("component_type", ""))
        if "join" in component_type:
            ops.append({"op": "mutate_param", "node_id": node["id"], "payload": {"join_type": "inner"}})
        if "filter" in component_type:
            ops.append({"op": "mutate_param", "node_id": node["id"], "payload": {"filter_scope": "dataset_row"}})
...
def _append_stability_fix_ops(
    ...
    if not any(kw in lower for kw in ("brittle", "gradient", "unstable", "nan", "explod", "stabil", "zero grad")) or ops:
        return
    existing_types = {str(n.get("component_type", "")).split("/")[-1] for n in nodes}
    if any(name in existing_types for name in ("layernorm", "layernorm_pre", "rmsnorm", "rmsnorm_pre")):
        return
    ...
    if output_node and output_node["id"] in incoming_edges:
        ...
        ops.append({"op": "rewire", "payload": {"action": "remove", "source": incoming["source"], "target": output_node["id"]}})
        ops.append({"op": "add_node", "payload": {
            **payload,
            "edges": [
                {"source": incoming.get("source", ""), "source_port": incoming.get("source_port", "out"), "target": node_id, "target_port": "in"},
                {"source": node_id, "source_port": "out", "target": output_node["id"], "target_port": incoming.get("target_port", "in")},
            ],
        }})
        return
    if last_node:
        ops.append({"op": "add_node", "payload": {
            **payload,
            "edges": [{"source": last_node["id"], "source_port": "out", "target": node_id, "target_port": "in"}],
        }})
...
def _append_split_branch_ops(
    ...
    if not any(kw in lower for kw in ("split pipeline", "parallel branch", "split branch", "parallelize", "two branches")) or ops:
        return
    output_node = next((n for n in nodes if "output" in str(n.get("component_type", ""))), None)
    ...
    ops.append({"op": "rewire", "payload": {"action": "remove", "source": trunk, "target": output_node["id"]}})
    ops.append({"op": "add_node", "payload": {"id": branch_id, "component_type": "math/relu", "params": {}, "ui_meta": {"position": {"x": 420, "y": 140}}, "edges": []}})
    ops.append({"op": "add_node", "payload": {"id": merge_id, "component_type": "math/add", "params": {}, "ui_meta": {"position": {"x": 560, "y": 220}}, "edges": [
        {"source": trunk, "source_port": incoming.get("source_port", "y"), "target": merge_id, "target_port": "a"},
        {"source": branch_id, "source_port": "y", "target": merge_id, "target_port": "b"},
        {"source": merge_id, "source_port": "y", "target": output_node["id"], "target_port": incoming.get("target_port", "x")},
    ]}})
    ops.append({"op": "rewire", "payload": {"action": "add", "source": trunk, "source_port": incoming.get("source_port", "y"), "target": branch_id, "target_port": "x"}})
...
def _select_fallback_component_type(
    ...
    component_type = None if is_benchmark_prompt else _infer_component_from_prompt(lower, suggestions)
    if component_type:
    ...
    if is_benchmark_prompt or "novelty" in lower:
        ...
        if not has_residual and len(nodes) > 3:
            non_io_nodes = [
                n for n in nodes
                if "input" not in str(n.get("component_type", "")) and "output" not in str(n.get("component_type", ""))
            ]
...
def _append_fallback_ops(
    ...
    if ops:
        return
    component_type = _select_fallback_component_type(lower, nodes, approved, suggestions, has_output, ops)
    ...
    if output_node and output_node["id"] in incoming_edges:
        incoming = incoming_edges[output_node["id"]]
        payload["edges"].append({"source": incoming.get("source", ""), "source_port": incoming.get("source_port", "out"), "target": node_id, "target_port": "in"})
        payload["edges"].append({"source": node_id, "source_port": "out", "target": output_node["id"], "target_port": incoming.get("target_port", "in")})
        ops.append({"op": "rewire", "payload": {"action": "remove", "source": incoming["source"], "target": output_node["id"]}})
    elif last_node is not None:
        payload["edges"].append({"source": last_node.get("id", ""), "source_port": "out", "target": node_id, "target_port": "in"})
    ops.append({"op": "add_node", "payload": payload})
...
def generate_patch_from_prompt(req: AskAriaPromptRequest) -> Dict[str, Any]:
    ...
    try: return _generate_patch_impl(req)
...
def _generate_patch_impl(req: AskAriaPromptRequest) -> Dict[str, Any]:
    ...
    _append_relative_insert_ops(
        ops, lower, nodes, incoming_edges, outgoing_edges, approved, suggestions,
    )
    ...
    _append_stability_fix_ops(
        ops, lower, workflow, nodes, incoming_edges, last_node, approved,
    )
    _append_split_branch_ops(ops, lower, nodes, incoming_edges)
    _append_fallback_ops(
        ops, lower, workflow, nodes, incoming_edges, last_node,
        approved, suggestions, has_output,
    )
...
def _fetch_parent_scores_for_workflow(workflow_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not notebook_path.exists(): return None
        nb = LabNotebook(str(notebook_path))
        try:
            res = nb.conn.execute("SELECT result_id FROM program_results WHERE graph_fingerprint = ? ORDER BY timestamp DESC LIMIT 1", (fingerprint,)).fetchone()
...
def refine_winner_endpoint(workflow_id: str, num_variations: int = 3, intent: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for pid in raw_proposal_ids:
            if len(valid_proposals) >= num_variations: break
            is_valid, error = _validate_proposal_quality(pid, parent_scores=parent_scores)
            if is_valid: valid_proposals.append(pid)
            else:
                logger.warning("Proposal %s failed quality gate: %s", pid, error)
                if error: rejection_reasons.append(error)
        return {"success": True, "generated_proposals": valid_proposals,
                "parent_scores_found": bool(parent_scores),
                "validation_failures": len(raw_proposal_ids) - len(valid_proposals),
                "warning": ("All refinement candidates regressed against the parent guardrails."
                            if raw_proposal_ids and not valid_proposals else None),
                "rejection_reasons": rejection_reasons[:5]}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


def _load_proposal_workflow_and_ops(proposal_id: str) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]], List[Dict[str, Any]], Optional[str]]:
...
def _compile_patched_refinement(workflow: Dict[str, Any], ops: List[Dict[str, Any]]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
...
def _validate_snapshot_guardrails(
    ...
    if parent_snapshot.get("has_gradient_path") and not candidate_snapshot.get("has_gradient_path"):
...
def _count_non_io_ops(workflow: Dict[str, Any]) -> int:
    return len([
        ...
        if str(node.get("component_type", "")).split("/")[0] not in ("graph_input", "graph_output", "io")
    ])
...
def _validate_parent_regression_guardrails(
    ...
    if parent_tier not in ("investigation", "validation", "breakthrough") or parent_composite <= 100:
        return None
    ...
    predicted = parent_composite * _estimate_patch_quality_multiplier(ops, parent_scores)
...
def _validate_proposal_quality(proposal_id: str, parent_scores: Optional[Dict[str, Any]] = None) -> Tuple[bool, Optional[str]]:
    ...
    guard_error = _validate_snapshot_guardrails(parent_snapshot, candidate_snapshot, parent_scores)
    if guard_error:
        return False, guard_error
    regression_error = _validate_parent_regression_guardrails(workflow, ops, parent_scores)
    if regression_error:
        return False, regression_error
    return True, None

def _build_refinement_quality_snapshot(workflow_json: Dict[str, Any]) -> Dict[str, Any]:
    snapshot: Dict[str, Any] = {"valid": True, "n_ops": 0, "depth": 0, "has_gradient_path": True, "smoke_test": None}
    try:
        if HAS_BRIDGE:
            ...
            snapshot.update({"n_ops": int(gi.get("n_ops") or 0), "depth": int(gi.get("depth") or 0),
...
def list_proposals(status: Optional[str] = Query(None), workflow_id: Optional[str] = Query(None),
                   fresh_only: bool = Query(False)) -> List[Dict[str, Any]]:
