...
def _build_component_indexes(
    ...
    Dict[str, List[Dict[str, Any]]], Dict[str, Dict[str, Any]], List[Dict[str, Any]]
]:
...
def suggest_components(
    ...
    if not leaf_nodes:
        ...
        suggestions.extend(
            _suggest_from_prompt(by_category, by_id, prompt, context=ctx)
        )
    ...
    suggestions.extend(
        _suggest_for_leaf_nodes(leaf_nodes, nodes, edges, by_category, ctx)
    )
    suggestions.extend(
        _suggest_graph_gaps(all_types, nodes, by_category, by_id, prompt, ctx)
    )

    if "io" not in categories_present and len(nodes) > 0:
        suggestions.extend(
            _suggest_component_ids(
                by_id,
                ["output_head"],
    ...
    ranked = sorted(
        _dedupe_suggestions(suggestions), key=lambda s: s.get("score", 0), reverse=True
    )
    return ranked[:5]
...
def _suggest_graph_gaps(
    ...
    suggestions: List[Dict[str, Any]] = []
    has_norm = any(
        t in all_types for t in ("layernorm", "layernorm_pre", "rmsnorm", "rmsnorm_pre")
    )
    has_attn = any(
        t in all_types
    ...
    if is_stability_prompt and not has_norm:
        suggestions.insert(
            ...
            _make_suggestion(
                by_id.get("rmsnorm")
                or next((c for c in by_category.get("normalization", [])), {}),
                ...
                score=0.99,
                evidence=[
                    "Prompt indicates stability concern",
                    "No normalization operators detected",
                ],
                context=ctx,
            ),
        )
        ...
        suggestions.insert(
            ...
            _make_suggestion(
                ...
                score=0.94,
                evidence=[
                    "Prompt indicates gradient instability",
                    "No residual add operator found",
                ],
                context=ctx,
            ),
        )
    ...
    if not has_norm:
        suggestions.extend(
            _suggest_category(
                by_category,
                "normalization",
                "Add normalization for training stability.",
                score=0.76,
                evidence=["No normalization category detected in graph"],
                context=ctx,
                exclude_ids={"no_norm", "none"},
            )
        )
    if not has_attn and len(nodes) > 3:
        suggestions.extend(
            _suggest_category(
                by_category,
                "mixing",
                "Add a mixing/attention layer for richer representations.",
                score=0.72,
                evidence=[
                    "Graph depth suggests representation bottleneck",
                    "No attention-like mixing found",
                ],
                context=ctx,
            )
        )
...
def _suggest_component_ids(
    ...
    for component_id in component_ids:
        ...
        if component is not None:
            out.append(
                _make_suggestion(
                    component, reason, score=score, evidence=evidence, context=context
                )
            )
    return out
...
def _suggest_category(
    ...
    for c in by_cat.get(category, []):
        ...
        if cid in excluded:
            continue
        res.append(
            _make_suggestion(c, reason, score=score, evidence=evidence, context=context)
        )
...
def _score_adjustment(
    ...
    if any(
        tok in prompt_lower
        for tok in ("stabil", "nan", "gradient", "brittle", "explod")
    ):
...
def _get_leaderboard_component_sets(
    ...
    parsed_sets = [
        _extract_component_ids_from_entry(entry)
...
def _extract_component_ids_from_entry(entry: Dict[str, Any]) -> Set[str]:
    ...
    if isinstance(graph_json, str) and graph_json:
        try:
            ...
            logger.debug(
                "Failed to parse graph_json in leaderboard entry", exc_info=True
            )
            graph = None
        if isinstance(graph, dict):
            ...
            if isinstance(nodes, list):
                return {
                    ...
                    and str(node.get("component_type", "")).strip()
                }

    text = str(
        entry.get("program_text") or entry.get("architecture_desc") or ""
    ).lower()
    if not text:
        return set()
    token_counts = Counter(
        _canon_leaf(token) for token in text.replace("/", " ").replace(",", " ").split()
    )
...
def _make_suggestion(
    ...
    adjusted_score = float(score) + _score_adjustment(
        component, reason, evidence_list, context=context
    )
