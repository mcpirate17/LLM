...
def _param_issues(param_name: str, schema: Dict[str, Any]) -> List[PropertyIssue]:
    ...
    if "type" not in schema:
        issues.append(
            PropertyIssue(
                "param_missing_type", f"Param '{param_name}' is missing type", "error"
            )
        )
    if "default" not in schema:
        issues.append(
            PropertyIssue(
                "param_missing_default", f"Param '{param_name}' is missing default"
            )
        )
    if schema.get("type") == "enum" and not schema.get("options"):
        issues.append(
            PropertyIssue(
                "enum_missing_options",
                f"Param '{param_name}' is enum without options",
                "error",
            )
        )
    if not schema.get("description"):
        issues.append(
            PropertyIssue(
                "param_missing_description", f"Param '{param_name}' has no description"
            )
        )
    return issues
...
def analyze_manifest(manifest: Dict[str, Any], source_path: str = "") -> Dict[str, Any]:
    ...
    if not manifest.get("description"):
        issues.append(
            PropertyIssue("missing_description", "Component has no description")
        )

    if not isinstance(params, dict):
        issues.append(
            PropertyIssue(
                "params_not_object", "Manifest params is not a mapping", "error"
            )
        )
        params = {}
        ...
        if not isinstance(schema, dict):
            issues.append(
                PropertyIssue(
                    ...
                    f"Param '{name}' schema is not an object",
                    "error",
                )
            )
            continue
    ...
    if _is_sink_component(manifest) and len(params) == 0:
        issues.append(
            PropertyIssue(
                "sink_missing_config",
                "Sink/output component should expose output configuration",
            )
        )
    ...
    if not manifest.get("inputs") and len(params) == 0:
        issues.append(
            PropertyIssue(
                "source_missing_config",
                "Source component has no configurable parameters",
            )
        )
