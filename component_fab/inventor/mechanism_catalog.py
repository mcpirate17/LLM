...
DEFAULT_INVENTION_BLUEPRINTS: tuple[InventionBlueprint, ...] = (
    ...
    InventionBlueprint(
        mechanism_id="signed_expander_mixer",
        category=CATEGORY_LANE,
        axes={
            "op_invention_mechanism": "signed_expander_mixer",
            "op_algebraic_space": "signed_expander",
            "op_dynamical_has_state": 1,
            "op_dynamical_memory_length_class": "O(L)",
            "op_activation_sparsity_pattern": "structured",
            "op_geometric_receptive_field": "global",
            "op_spectral_preferred_basis": "expander",
            "op_param_efficiency_family": "MiniMax-M3-align M3X-M2",
        },
        information_flow="each token reads a causal decayed context, then a signed regular channel-expander spreads that state through a fixed sparse graph with learned signed edge weights",
        forgetting_rule="per-channel geometric decay on the causal context; the expander has no convex token averaging or learned attention weights",
        causality_argument="context is a strict left-to-right scan and channel expansion is pointwise at token t, so output at t depends only on tokens <= t",
        target_failure_mode="dense O(D^2) mixers waste parameters while narrow sparse bands fail to spread information globally; expander mixing gives global breadth with O(D * degree) footprint",
        expected_baseline="sparse_banded_matrix",
        complexity="O(L * D * degree) with O(D + degree) parameters",
    ),
    InventionBlueprint(
        mechanism_id="auction_capacity_router",
        category=CATEGORY_ROUTING,
        axes={
            "op_invention_mechanism": "auction_capacity_router",
            "op_algebraic_space": "auction_assignment",
            "op_dynamical_has_state": 1,
            "op_dynamical_memory_length_class": "O(L)",
            "op_activation_sparsity_pattern": "hard_top1_capacity",
            "op_geometric_receptive_field": "causal_prefix",
            "op_spectral_preferred_basis": "content",
            "op_routing_kind": "auction_capacity",
            "op_param_efficiency_family": "MiniMax-M3-align M3X-R1",
        },
        information_flow="tokens bid for experts with raw non-softmax scores; a causal left-to-right auction subtracts expert prices and masks full experts, then routes each token to one hard-capacity expert",
        forgetting_rule="expert prices rise as an expert receives tokens, preventing repeated collapse into one lane; hard capacity masks enforce bounded load",
        causality_argument="prices and loads are updated only from previous positions in the same sequence, so route decisions at t depend only on tokens <= t",
        target_failure_mode="softmax router collapse where a gate abandons novel experts; auction capacity makes balanced expert use a structural property instead of an auxiliary-loss hope",
        expected_baseline="top_k_moe",
        complexity="O(L * K * D) for K experts plus O(L * K) causal auction state",
    ),
    InventionBlueprint(
    ...
    InventionBlueprint(
        mechanism_id="holographic_binding",
        category=CATEGORY_LANE,
        axes={
            "op_invention_mechanism": "holographic_binding",
            "op_algebraic_space": "holographic_reduced_representation",
            "op_dynamical_has_state": 1,
            "op_dynamical_memory_length_class": "O(L)",
            "op_activation_sparsity_pattern": "dense",
            "op_geometric_receptive_field": "global",
        ...
        information_flow="each token binds a unit key to a value by circular convolution (FFT product), superposes the bound pairs into a causal decayed memory, and retrieves by circular correlation (approximate unbind) with a query — VSA/HRR associative memory, no query/key attention scores",
        forgetting_rule="per-channel geometric decay on the superposed holographic memory",
        causality_argument="the memory is a strict causal decayed sum of bound key/value pairs over tokens <= t",
        target_failure_mode="compositional multi-slot binding (2+ key/value triples) where non-QKV mixers collapse to chance; algebraic bind/unbind isolates key-specific values up to VSA capacity D/#pairs",
        expected_baseline="causal_slot_router_memory",
        complexity="O(L * D log D) for the FFT bind/unbind; O(D) state",
    ),
)
