...
def _drive_loop(args: argparse.Namespace, ledger: Ledger, proposals_path: Path) -> list[dict]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.quiet:
        promoted = sum(
            1 for entry in ledger.all_entries() if entry.promotion_status == PROMOTION_PROMOTED
        )
        print(
