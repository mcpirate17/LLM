"""Tests for WS-4 (Pareto/niche promotion) + WS-5 (operational-spectrum orthogonality)."""
...
# --------------------------------------------------------------------------- #
# WS-5: operational spectrum + orthogonality radius
# --------------------------------------------------------------------------- #
def test_spectrum_reads_scorecards():
    probe = {"aggregate_loss_ratio": 100.0, "per_task": {}}
    cap = {"erf_density": 0.3, "nb_max_accuracy": 0.8, "ind_max_accuracy": 0.5}
    fp = operational_spectrum(probe, cap)
    assert fp["nb_max_accuracy"] == 0.8
    assert fp["ind_max_accuracy"] == 0.5
    assert fp["learning"] > 0.0  # log10(100)/2 = 1.0


def test_identical_behavior_is_degenerate_regardless_of_axes():
    # Same operational spectrum, different axes -> behavioral degeneracy.
    fp_a = operational_spectrum(
    ...
    dist = orthogonality_radius(fp_b, catalog)
    assert dist == 0.0
    assert is_degenerate(dist)


def test_distinct_behavior_not_degenerate():
    fp_a = operational_spectrum({"aggregate_loss_ratio": 2.0}, {"nb_max_accuracy": 0.1})
    fp_b = operational_spectrum(
        {"aggregate_loss_ratio": 100.0},
        {"nb_max_accuracy": 0.95, "ind_max_accuracy": 0.9},
    )
    fp_c = operational_spectrum({"aggregate_loss_ratio": 1.0}, {"erf_density": 0.01})
    catalog = [fp_a, fp_b]
    dist = orthogonality_radius(
        fp_c, catalog, normalizer=Normalizer.fit([fp_a, fp_b, fp_c])
    )
    assert not is_degenerate(dist, eps=DEFAULT_DEGENERACY_EPS)


def test_empty_catalog_is_infinitely_orthogonal():
    fp = operational_spectrum({"aggregate_loss_ratio": 5.0}, {})
    assert orthogonality_radius(fp, []) == float("inf")


def test_spectrum_from_metadata_coarse_fallback():
    fp = spectrum_from_metadata(
...
def test_orthogonality_requires_stability_floor():
    # A "distinct but useless" candidate (no capability, high orthogonality) must
    # NOT ride the orthogonality axis onto the front: it is floored to 0 so a
    # capable candidate dominates it. Mission: the loop must not keep junk just
    # because it is behaviorally distinct.
    useless_distinct = objective_vector(
        {"aggregate_loss_ratio": 1.0}, {"ind_max_accuracy": 0.0}, orthogonality=5.0
    )
    assert useless_distinct["orthogonality"] == 0.0
    capable = objective_vector(
        {"aggregate_loss_ratio": 100.0},
        {
            "binds_per_probe": {"p": True},
            "relative_recall_per_probe": {"p": 0.9},
    ...
    fronts = non_dominated_sort([useless_distinct, capable])
    assert fronts[1] == 0  # capable is non-dominated
    assert fronts[0] == 1  # useless-distinct now dominated (no axis where it wins)


def test_capable_candidate_keeps_full_orthogonality():
    # A capable candidate that is ALSO distinct keeps its orthogonality in full —
    # the floor never penalizes a genuine win.
    capable_distinct = objective_vector(
        {"aggregate_loss_ratio": 50.0},
        {"binds_per_probe": {"p": True}, "relative_recall_per_probe": {"p": 0.8}},
        orthogonality=3.0,
    )
    assert capable_distinct["orthogonality"] == 3.0


# --------------------------------------------------------------------------- #
# Shared stability-floored anti-twin (scalar composite + Pareto, one source)
# --------------------------------------------------------------------------- #
def test_stability_floor_credits_any_axis_not_just_binding():
    # Mission: a non-QKV state-tracking specialist (binding≈0) must still clear the
    # floor on its winning axis, so the floor maxes over ALL capability dims.
    assert meets_stability_floor(
        binding=0.0, induction=0.0, learning=0.0, state_tracking=0.2
    )
    assert meets_stability_floor(
        binding=0.0, induction=0.2, learning=0.0, state_tracking=0.0
    )
    # all axes dead -> no orthogonality credit
    assert not meets_stability_floor(
        binding=0.0, induction=0.0, learning=0.0, state_tracking=0.0
    )
    # every axis just below the floor -> still not stable
    assert not meets_stability_floor(
        binding=0.04, induction=0.04, learning=0.04, state_tracking=0.04
    )


def _solo_with_orthogonality(radius: float) -> dict:
    return {
        "smoke": {
            "forward_passed": True,
        ...
        "metadata": {"orthogonality_radius": radius},
    }


def test_composite_orthogonality_lift_gated_by_stability_floor():
    probe = {"aggregate_loss_ratio": 1.0}  # learning 0
    strong = {"binds_per_probe": {"p": True}, "relative_recall_per_probe": {"p": 1.0}}
    dead = {"ind_max_accuracy": 0.0}

    # Capable candidate: full orthogonality lift (radius 4.0 -> 1.0 -> +MAX).
    capable_distinct, _ = composite_score(_solo_with_orthogonality(4.0), probe, strong)
    capable_plain, _ = composite_score(_solo_with_orthogonality(0.0), probe, strong)
    assert abs((capable_distinct - capable_plain) - _ORTHOGONALITY_LIFT) < 1e-9

    # Useless-but-distinct candidate: no lift, despite identical radius.
    useless_distinct, _ = composite_score(_solo_with_orthogonality(4.0), probe, dead)
    useless_plain, _ = composite_score(_solo_with_orthogonality(0.0), probe, dead)
    assert useless_distinct == useless_plain
...
def test_annotate_and_finalize_wires_niche_metadata(tmp_path: Path):
    ...
    for surv in survivors:
        ...
        assert "orthogonality_radius" in md
