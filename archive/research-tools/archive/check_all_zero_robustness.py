...
rows = conn.execute(
    "SELECT experiment_id, results_json FROM experiments WHERE experiment_type = 'investigation'"
).fetchall()
...
for rid in zero_robustness_ids[:10]:
    row = conn.execute(
        "SELECT investigation_robustness FROM leaderboard WHERE result_id = ?", (rid,)
    ).fetchone()
    if row:
