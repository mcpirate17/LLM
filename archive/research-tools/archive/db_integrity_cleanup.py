...
def cleanup():
    ...
    cur = conn.execute(
        "DELETE FROM program_results WHERE experiment_id NOT IN (SELECT experiment_id FROM experiments)"
    )
    if cur.rowcount > 0:
    ...
    cur = conn.execute(
        "DELETE FROM experiments WHERE n_programs_generated = 0 AND status != 'running'"
    )
    if cur.rowcount > 0:
    ...
    cur = conn.execute(
        "DELETE FROM training_curves WHERE result_id NOT IN (SELECT result_id FROM program_results)"
    )
    if cur.rowcount > 0:
    ...
    cur = conn.execute(
        "DELETE FROM leaderboard WHERE result_id NOT IN (SELECT result_id FROM program_results)"
    )
    if cur.rowcount > 0:
