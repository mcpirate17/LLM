...
row = nb.conn.execute(
    "SELECT investigation_robustness FROM leaderboard WHERE result_id = ?", (result_id,)
).fetchone()
...
row = nb.conn.execute(
    "SELECT investigation_robustness, investigation_passed FROM leaderboard WHERE result_id = ?",
    (result_id,),
).fetchone()
