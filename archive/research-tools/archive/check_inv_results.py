...
for exp_id in experiments:
    row = conn.execute("SELECT results_json FROM experiments WHERE experiment_id = ?", (exp_id,)).fetchone()
    if row and row['results_json']:
        try:
            ...
            for r in inv_res:
                print(f"Source ID: {r.get('result_id')}, Robustness: {r.get('robustness')}, Best LR: {r.get('best_loss_ratio')}")
