...
def clean_knowledge_base():
    ...
    cursor.execute(
        "SELECT insight_id, content FROM insights WHERE status IN ('active', 'superseded')"
    )
    ...
    if exact_duplicates:
        ...
        with conn:
            for i in range(0, len(exact_duplicates), 500):
                ...
                cursor.execute(
                    f"UPDATE insights SET status = 'archived_duplicate' WHERE insight_id IN ({placeholders})",
                    batch,
                )
    ...
    logger.info(
        f"Analyzing {len(remaining_rows)} unique insights for fuzzy redundancy (sampling top 60)..."
    )
...
"""
    ...
    if to_delete:
        logger.info(
            f"Fuzzy deduplication identified {len(to_delete)} candidates. Archiving..."
        )
        with conn:
