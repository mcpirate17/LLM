...
def register_reference(
    ...
    if ref_name in existing_refs:
        ...
        return {"arch": arch_key, "status": "already_registered", "reference_name": ref_name}
    ...
    sandbox_result = safe_eval(model, batch_size=2, seq_len=seq_len,
                               vocab_size=vocab_size, device=str(dev),
                               run_stability_probe=True)
    if not sandbox_result.passed:
        if sandbox_result.error_type == "zero_grad":
            log.warning("  safe_eval zero-grad (likely saturated identity); trying shifted targets...")
            model.zero_grad()
            ...
            if _nonzero > 0:
                log.info("  Shifted-target check passed: %d/%d nonzero grads", _nonzero, len(_grads))
                model.zero_grad()
            else:
                log.error("  Stage 0 FAILED for %s: zero grads even with shifted targets", arch_key)
                return {"arch": arch_key, "status": "stage0_failed", "error": "zero_grad_shifted"}
        else:
    ...
    try:
        ...
        log.info("  Fingerprint: novelty=%.4f, locality=%.4f, isotropy=%.4f",
                 fp_novelty, bfp.interaction_locality, bfp.isotropy)
    ...
    try:
        ...
        log.info("  Novelty: overall=%.4f, structural=%.4f, behavioral=%.4f, similar_to=%s",
                 overall_novelty, structural_novelty, behavioral_novelty, most_similar)
    except Exception as e:
    ...
    try:
        ...
        log.info("  FLOPs: forward=%s, per_param=%.2f", f"{flops_fwd:,}", param_efficiency)
    except Exception as e:
    ...
    try:
        ...
        if quant_result:
            ...
            log.info("  Quant INT8 retention: %s, quality/byte: %s",
                     quant_retention, quant_quality_per_byte)
    except Exception as e:
    ...
    try:
        for seed in [42, 123, 789]:
            ...
            seed_model = compile_model(seed_layers, vocab_size=vocab_size, max_seq_len=seq_len).to(dev)
            seed_lr, seed_curve = _micro_train(seed_model, str(dev), vocab_size, seq_len,
        ...
        log.info("  Init sensitivity std: %.6f (losses: %s)", init_sensitivity_std,
    ...
    log.info("  Baseline ratio: %.4f (baseline_loss=%.4f)", baseline_ratio, baseline_loss)
    ...
    if nb.has_fingerprint(graph_fp):
        log.info("  Architecture already exists in program_results (fp=%s), using existing result...", graph_fp)
    ...
    upsert_kwargs = dict(
        result_id=result_id, model_source="reference",
        architecture_desc="%s: %s" % (ref_name, ref_info["description"]),
        screening_loss_ratio=loss_ratio, screening_novelty=overall_novelty,
        screening_passed=True,
        investigation_loss_ratio=loss_ratio, investigation_robustness=1.0,
    ...
    return {"arch": arch_key, "status": "registered", "entry_id": entry_id,
            "reference_name": ref_name, "loss_ratio": loss_ratio,
            "baseline_ratio": baseline_ratio, "param_count": total_params,
            "fingerprint": graph_fp, "novelty": overall_novelty,
            "noise_score": noise_score, "quant_retention": quant_retention,
            "init_sensitivity_std": init_sensitivity_std}
...
def _micro_train(model, device, vocab_size, seq_len, n_steps, lr=3e-4, batch_size=4, data_mode="random"):
    ...
    return loss_curve[-1] / loss_curve[0] if loss_curve[0] > 0 else float("inf"), loss_curve
...
def main():
    ...
    for key in arch_keys:
        r = register_reference(key, device=args.device, d_model=args.d_model,
                               n_layers=args.n_layers, n_train_steps=args.n_steps)
