...
| `export_induction_ml_dataset.py` | pipeline | Export one-row-per-fingerprint induction dataset for ML and analysis | `python -m research.tools.export_induction_ml_dataset` | - |
| `analyze_pass_vs_induction.py` | pipeline | Compare stage-1 pass rates against canonical induction learning labels | `python -m research.tools.analyze_pass_vs_induction` | - |
...
| `audit_tools_surface.py` | integrity | Inventory and classify every `research.tools` entrypoint and DB touchpoint | `python -m research.tools.audit_tools_surface` | - |
