...
def slow_recurrent_slotted_memory(q, k, v, write_routes):
    ...
    for t in range(seq_len):
        ...
        mask = torch.nn.functional.one_hot(w_idx, num_classes=n_slots).unsqueeze(-1).to(q.dtype)
...
def fast_parallel_slotted_memory(q, k, v, write_routes):
    ...
    mask = torch.nn.functional.one_hot(w_idx, num_classes=n_slots).to(q.dtype) # [B, L, Slots]
