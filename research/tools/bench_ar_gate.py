#!/usr/bin/env python
"""Micro-benchmark for ar_gate hot paths (CPU).

Three sections:
  A. class_token_ids — the inline 45-encode loop in ar_gate()
  B. corpus tensor build — tokenize 595 sentences + 15 prompts, pack to tensors
  C. end-to-end ar_gate on a tiny graph (mirrors test_probe_smoke_on_tiny_graph)

Reports median over N reps. Each section also reports per-sub-step counters.

Usage:
    python -m research.tools.bench_ar_gate --reps 5
    python -m research.tools.bench_ar_gate --reps 5 --no-e2e   # skip C
"""
...
def _bench_class_token_ids_inline(reps: int) -> list[float]:
    ...
    from research.eval.ar_gate import _tokenize_words
...
def _bench_class_token_ids_cached(reps: int) -> list[float]:
    ...
    from research.eval import ar_gate as _mod
    from research.eval.ar_gate import ARGateConfig, _get_class_token_ids
    from research.eval.utils import _get_tiktoken_encoder
...
def _bench_end_to_end(reps: int, device: str) -> list[float]:
    ...
    for _ in range(reps):
        ...
        if device == "cuda":
            torch.cuda.synchronize()
        t0 = time.perf_counter()
        result = ar_gate(graph_json=graph_to_json(g), device=device, cfg=cfg)
        if device == "cuda":
            torch.cuda.synchronize()
        times.append((time.perf_counter() - t0) * 1000)
...
def main() -> None:
    ...
    print("=" * 70)
    print(f"ar_gate benchmark (device={args.device}, reps={args.reps})")
    print("=" * 70)
    ...
    if not args.no_e2e:
        print(
            f"\n[C] end-to-end ar_gate (tiny graph, 20 finetune steps, {args.device})"
        )
        c = _bench_end_to_end(args.reps, args.device)
        _summarize("ar_gate full", c)
