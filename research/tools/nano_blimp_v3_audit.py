"""Real-architecture cohort audit for nano_blimp_v3 (real-word held-out).

Mirrors ``nano_blimp_v2_audit.py`` but drives ``nano_blimp_v3_score`` —
the real-word probe over codex's curated noun/verb lists. Reports each
held-out component separately so SSM-class candidates that lag on
positional ordering but learn the binding rule remain visible.

Read-only: no DB writes, no leaderboard mutation.

Usage:
    python -m research.tools.nano_blimp_v3_audit \
        --targets ec7025d7-338 574271ca-f37 f70c17d0-d59 8d087a16-692 903157e5-219 \
        --seeds 1 2 3 \
        --held-out 2 4 6 \
        --n-per-type 24 --probe-steps 200 \
...
from research.tools.nano_blimp_v2_audit import _load_arch  # reuse
...
def _print_summary_table(summary: list[dict[str, Any]]) -> None:
    print("\n=== nano_blimp_v3 (real-word) cohort audit (mean ± std) ===")
    header = (
...
def main() -> int:
    ...
    for rid in args.targets:
        ...
        if not meta or not meta.get("graph_json"):
            log.warning("no graph_json for %s; skipping", rid)
            continue
    ...
    log.info("wrote %d runs -> %s", len(runs), args.out)
    return 0
