#!/usr/bin/env python3
"""Concurrent probe backfill using subprocess workers.

Generic runner for any --post-train-target supported by
backpopulate_screening_metrics (hellaswag, induction, binding, ar, blimp, ncd).

Usage:
    python -m research.tools.run_probe_backfill \
        --run-dir research/reports/blimp_fastpath_all_c12_2026-04-11 \
        --probe blimp --concurrency 12 --device cuda
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Concurrent probe backfill")
    p.add_argument("--run-dir", required=True, type=Path)
    p.add_argument(
        "--probe",
        required=True,
        help="Post-train target: hellaswag, induction, binding, ar, blimp, ncd",
    )
...
def _launch_worker(run_dir: Path, result_id: str, probe: str, device: str) -> dict:
        ...
        while pending or active:
            ...
            if finished:
                ...
                print(
                    ...
                    f"active={len(active)} rate={rate:.1f}/s ETA={eta / 60:.0f}min",
                    flush=True,
                )
                status_payload = {
