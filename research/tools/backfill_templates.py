...
_TEMPLATE_BACKFILL_POLICIES: dict[str, TemplateBackfillPolicy] = {
    ...
    "sparsemax_attention_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=12,
        max_batch=24,
        max_retries=3,
        preferred_weight_mode="uniform",
        zero_s1_stop_after_s0=10,
        notes="New sparse-retrieval attention primitive; gather neutral coverage before harvest.",
    ),
    "entmax_attention_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=12,
        max_batch=24,
        max_retries=3,
        preferred_weight_mode="uniform",
        zero_s1_stop_after_s0=10,
        notes="New tunably sparse attention primitive; pair with sparsemax coverage for attribution.",
    ),
    "dplr_gated_delta_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=12,
        max_batch=24,
        max_retries=3,
        preferred_weight_mode="scaffold_guided",
        zero_s1_stop_after_s0=10,
        notes="New DPLR/delta recurrent memory primitive; scaffold-guided coverage should preserve stable recurrence wrappers.",
    ),
    "retention_mix_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=12,
        max_batch=24,
        max_retries=3,
        preferred_weight_mode="scaffold_guided",
        zero_s1_stop_after_s0=10,
        notes="New retentive mixer; run alongside DPLR delta for recurrence-family attribution.",
    ),
    "token_hodge_mixer_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=8,
        max_batch=16,
        max_retries=2,
        preferred_weight_mode="uniform",
        zero_s1_stop_after_s0=8,
        notes="Novel topology-aware mixer; cap early batches until stability and S1 conversion are known.",
    ),
    "wavelet_packet_mix_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=8,
        max_batch=16,
        max_retries=2,
        preferred_weight_mode="uniform",
        zero_s1_stop_after_s0=8,
        notes="New multiscale wavelet mixer; previous wavelet-only probes were mixed, so collect cautious coverage.",
    ),
    "product_key_memory_block": TemplateBackfillPolicy(
        mode="coverage",
        min_batch=8,
        max_batch=16,
        max_retries=2,
        preferred_weight_mode="uniform",
        zero_s1_stop_after_s0=8,
        notes="High-capacity product-key memory primitive; cap first pass to watch parameter/memory pressure.",
    ),
