...
Runs nano-AR-INV on the top-20 archs (mix of nano_bind passers + failers)
    ...
    return [
        {
            ...
            "templates": json.loads(r[3]) if r[3] else [],
...
def main() -> None:
    ...
    for i, arch in enumerate(cohort, start=1):
        ...
        held_class = []
        for sd in SEEDS:
            cfg = NanoARInvConfig(seed=sd, **CFG_KW)
            r = nano_ar_inv(graph_json=arch["graph_json"], device="cuda", cfg=cfg)
            in_pair.append(r.in_dist_pair_match_acc)
    ...
    json_path = REPORTS_DIR / "nano_ar_inv_top20_heavy_warmup_3seed.json"
    md_path = REPORTS_DIR / "nano_ar_inv_top20_heavy_warmup_3seed.md"
    json_path.write_text(json.dumps(out, indent=2, default=str))
    rows_sorted = sorted(out_rows, key=lambda r: r["in_pair_mu"], reverse=True)
    lines = [
        "# nano-AR-INV top-20 heavy-warmup 3-seed sweep",
        "",
