...
rows = conn.execute("SELECT experiment_id, results_json FROM experiments WHERE experiment_type = 'investigation' AND status = 'completed'").fetchall()
        ...
        try:
            ...
            for res in inv_results:
                ...
                if rid and robustness is not None:
                    ...
                    lb_row = conn.execute("SELECT entry_id, investigation_robustness FROM leaderboard WHERE result_id = ?", (rid,)).fetchone()
                    if lb_row:
                        if lb_row['investigation_robustness'] is None:
                            print(f"Backfilling {rid}: robustness={robustness}, best_lr={best_lr}")
                            ...
                            conn.execute("""
                                ...
                                WHERE result_id = ?
                            """, (best_lr, robustness, 1 if (robustness >= 0.5 and (best_lr or 1.0) < 0.5) else 0, rid))
