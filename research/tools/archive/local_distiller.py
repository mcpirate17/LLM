...
class LocalDistiller:
    ...
    def cluster_failures(self):
        ...
        cursor.execute("SELECT signature, error_types, n_failures FROM failure_signatures WHERE n_failures >= 2")
        ...
        sig_data = []
        for s in signatures:
            sig_data.append(f"Types: {s['error_types']} | Count: {s['n_failures']} | Sig: {s['signature'][:200]}")
...
"""
        ...
        try:
            ...
            with open("research/artifacts/failure_clusters.json", "w") as f:
                f.write(clean_json)
            logger.info("Saved failure clusters to research/artifacts/failure_clusters.json")
        except Exception as e:
    ...
    def generate_narratives(self, limit: int = 50):
        ...
        for row in rows:
            ...
            ops = [node['op_name'] for node in graph['nodes'].values() if not node.get('is_input')]
...
"""
        ...
        logger.info("Saved narratives to research/artifacts/architecture_narratives.json")
