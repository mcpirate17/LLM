#!/usr/bin/env python
"""Calibration harness for the Small-AR champion probe.

The harness sweeps ``SmallARChampionConfig`` difficulty knobs against compact
reference models and writes read-only artifacts under
``research/runtime/small_ar_calibration/``.  It does not mutate the notebook DB.
"""
...
CALIBRATION_PROTOCOL_VERSION = "small_ar_champion_calibration_v1"
SELECTED_CONFIG_NAME = "calibrated_v2_default"
SWEEP_FIELDS = (
    "n_key_tokens",
    "n_value_tokens",
    "n_value_classes",
...
def selected_small_ar_config(**overrides: Any) -> SmallARChampionConfig:
    """Return the calibrated v2 default, with optional run-local overrides."""
...
def default_sweep_configs(
    *, quick: bool = False
) -> list[tuple[str, SmallARChampionConfig]]:
    """Difficulty sweep covering the requested Small-AR knobs."""
    if quick:
        steps = 600
        eval_every = 200
        n_eval = 256
...
def _required_vocab_size(cfg: SmallARChampionConfig) -> int:
    return int(cfg.vocab_lo) + int(cfg.n_key_tokens) + int(cfg.n_value_tokens) + 2


def _seq_len(cfg: SmallARChampionConfig) -> int:
...
def _result_row(
    *,
    ...
    return {
        ...
        "metric_version": SMALL_AR_CHAMPION_METRIC_VERSION,
        ...
        "held_pair_match_acc": result.held_pair_match_acc,
...
def run_calibration_grid(
    configs: list[tuple[str, SmallARChampionConfig]],
    *,
    ...
    for config_name, cfg in configs:
        for family in families:
            ...
            try:
                result = run_small_ar_champion(model, cfg=cfg, device=device)
            finally:
                del model
                if device == "cuda":
...
def select_calibrated_setting(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    for config_name, family_rows in by_config.items():
        ...
        attn_acc = float(attention["held_pair_match_acc"])
        noctx_acc = float(no_context["held_pair_match_acc"])
        ...
        candidates.append(
            {
                "config_name": config_name,
                "attention_held_pair_match_acc": attn_acc,
                "no_context_held_pair_match_acc": noctx_acc,
                "gap": round(gap, 6),
                "chance": chance,
    ...
    return max(
        ...
        key=lambda row: (
            ...
            row["gap"],
            row["attention_held_pair_match_acc"],
        ),
    )
...
def write_report(report: dict[str, Any], out_dir: Path) -> tuple[Path, Path]:
    ...
    json_path = out_dir / f"small_ar_calibration_{stamp}.json"
    md_path = out_dir / f"small_ar_calibration_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    lines = [
        f"# Small AR Calibration - {stamp}",
        "",
    ...
    for row in report["rows"]:
        ...
        lines.append(
            "| {config_name} | {model_family} | {held_pair_match_acc:.4f} | "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.override:
        ...
        cfg = replace(selected_small_ar_config(), **overrides)
    ...
    if selected is None:
        ...
        return 2
    print(
        "selected {config_name}: attention={attention_held_pair_match_acc:.4f} "
        "no_context={no_context_held_pair_match_acc:.4f} gap={gap:.4f} "
