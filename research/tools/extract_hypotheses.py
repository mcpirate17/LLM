...
def analyze_families(entries: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for e in entries:
        ...
        if e["score"] is not None:
            f_data["scores"].append(e["score"])
        if e["screening_lr"] is not None:
            f_data["screening_lrs"].append(e["screening_lr"])
        if e["validation_lr"] is not None:
            f_data["validation_lrs"].append(e["validation_lr"])
    ...
    for name, data in families.items():
        if data["count"] < 2:
            continue  # Need a cluster

        avg_score = sum(data["scores"]) / len(data["scores"]) if data["scores"] else 0
        avg_screening = (
            sum(data["screening_lrs"]) / len(data["screening_lrs"])
            if data["screening_lrs"]
            else 1.0
        )
        ...
        results.append(
            {
                ...
                "n_validated": data["tiers"].get("validation", 0)
                + data["tiers"].get("breakthrough", 0),
            }
        )
...
def main():
    ...
    try:
        ...
        with open(OUTPUT_MD, "w") as f:
            ...
            f.write(
                "Analysis of architectural families showing consistent high performance.\n\n"
            )

            f.write(
                "| Family | Count | Avg Score | Screening LR | Improv vs Baseline | Top Tier |\n"
            )
            f.write("| :--- | :--- | :--- | :--- | :--- | :--- |\n")

            for m in motifs:
                f.write(
                    f"| **{m['family']}** | {m['count']} | {m['avg_score']:.1f} | {m['avg_screening_lr']:.4f} | {m['improvement_pct']:+.1f}% | {m['top_tier']} |\n"
                )
            ...
            for m in motifs[:3]:
                if m["improvement_pct"] > 10:
                    f.write(
                        f"- **{m['family']}** shows significant potential with a average improvement of {m['improvement_pct']:.1f}% over baseline across {m['count']} candidates.\n"
                    )

        print(
            f"Successfully extracted {len(motifs)} motifs. Summary saved to {OUTPUT_MD}"
        )
