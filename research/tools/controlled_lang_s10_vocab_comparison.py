...
from research.tools.controlled_lang_backfill import _train_base
...
def _fetch_rows(
    ...
    rows = conn.execute(
        ...
        SELECT l.tier,
               ...
               pr.graph_json,
               pr.controlled_lang_s05_nb_score AS old_s05_nb,
               pr.controlled_lang_s10_nb_score AS old_s10_nb,
               pr.controlled_lang_s10_nb_order_acc AS old_s10_order
        FROM leaderboard l
        JOIN program_results pr ON pr.result_id = l.result_id
        WHERE pr.controlled_lang_s10_nb_score IS NOT NULL
...
def main() -> int:
    ...
    parser.add_argument(
        "--db", type=Path, default=REPO_ROOT / "research/lab_notebook.db"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=REPORTS_DIR
        / f"controlled_lang_s10_vocab240_comparison_{int(time.time())}.json",
    )
    ...
    for idx, row in enumerate(rows, 1):
        ...
        try:
            ...
            checkpoints = payload.get("controlled_lang_checkpoints") or []
