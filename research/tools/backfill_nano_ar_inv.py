#!/usr/bin/env python
"""Backfill ``nano_ar_inv_*`` columns on a fingerprint cohort.
...
For each row, runs ``nano_ar_inv(graph_json=..., from_s1=False)`` with the
...
created). Skips rows that already have ``nano_ar_inv_score IS NOT NULL``
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--force",
        action="store_true",
        help="Re-run rows that already have nano_ar_inv_score persisted.",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
...
def persist_result(conn: sqlite3.Connection, result_id: str, r) -> None:
    score = round(0.6 * r.in_dist_pair_match_acc + 0.4 * r.held_class_acc, 4)
    conn.execute(
        _UPDATE_SQL,
        (
            r.metric_version,
            r.in_dist_pair_match_acc,
            r.in_dist_class_acc,
            r.held_pair_match_acc,
            r.held_class_acc,
            score,
            r.status,
            r.elapsed_ms,
...
def main() -> None:
    ...
    for i, arch in enumerate(cohort, start=1):
        ...
        rid_short = rid[:12]
        try:
            r = nano_ar_inv(graph_json=arch["graph_json"], device=args.device, cfg=cfg)
        ...
        out_log.append(
            {
                ...
                "in_pair": r.in_dist_pair_match_acc,
                "in_class": r.in_dist_class_acc,
                "held_pair": r.held_pair_match_acc,
    ...
    for entry in out_log:
        ...
        bucket["n"] += 1
        bucket["score_sum"] += entry["nano_ar_inv_score"]
    ...
    out_path = REPORTS_DIR / f"nano_ar_inv_backfill_{int(time.time())}.json"
    out_path.write_text(
