#!/usr/bin/env python
"""Pilot AR gate-INV against the top-N leaderboard architectures.

Read-only against ``runs.db``. Writes a JSON + markdown summary to
...
AR gate corpus alone (no wikitext warmup) — fast signal of whether the
architecture has the retrieval mechanism at all.

Usage::

    python -m research.tools.ar_gate_pilot --top-n 30 --device cuda
    python -m research.tools.ar_gate_pilot --top-n 10 --device cpu --train-steps 200
"""
...
def fetch_top_archs(
    ...
    try:
        ...
        for rid, gj, comp, tier, ppl, ar, ind, bnd in rows:
            try:
                gj = resolve_graph_json_value(conn, db_path, gj)
                graph = json.loads(gj)
            ...
            out.append(
                {
                    ...
                    "ar_legacy_auc_legacy": float(ar) if ar is not None else None,
                    "induction_screening_auc": float(ind) if ind is not None else None,
                    "binding_screening_auc": float(bnd) if bnd is not None else None,
...
def write_reports(report: dict[str, Any], out_prefix: str) -> tuple[Path, Path]:
    ...
    rows_by_score = sorted(
        rows,
        key=lambda r: r["ar_gate"]["in_dist_pair_acc"],
        reverse=True,
    )
    cfg = report["config"]
    lines = [
        f"# AR gate-INV pilot — {report['n_archs']} arches, {report['elapsed_s']}s wall",
        "",
    ...
    for r in rows_by_score:
        ...
        ar_legacy = r.get("ar_legacy_auc_legacy")
        ar_str = f"{ar_legacy:.3f}" if ar_legacy is not None else "—"
        lines.append(
            f"| {r['result_id'][:12]} | {tpl} | {r['composite_score']:.1f} | "
            f"{n['in_dist_pair_acc']:.3f} | {n['in_dist_class_acc']:.3f} | "
            f"{n['held_pair_acc']:.3f} | {n['held_class_acc']:.3f} | "
            f"{ar_str} | {n['status']} |"
        )
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    args = parse_args()
    out_prefix = args.out_prefix or f"ar_gate_pilot_{int(time.time())}"
