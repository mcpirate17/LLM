...
def _colab_code_for_probe(spec: ProbeSpec, limit: int) -> str:
    report_ext = "jsonl" if spec.name == "ar_gate" else "tsv"
    if spec.name == "ar_gate":
        command = """
cmd = [
    sys.executable,
    '-m',
    'research.tools.colab_ar_gate_backfill',
    '--device',
    'cuda',
    '--candidates-jsonl',
    str(CANDIDATE),
...
cmd = [
    sys.executable,
    '-m',
    'research.tools.colab_language_control_nb_backfill',
    '--probe',
    TARGET,
    '--device',
    'cuda',
...
cmd = [
    sys.executable,
    '-m',
    'research.tools.colab_nano_bind_backfill',
    '--device',
    'cuda',
    '--candidates-jsonl',
    str(CANDIDATE),
...
"""
    report_ext = (
        "jsonl" if spec.name in {"ar_gate", "nb05", "nb10", "nano_bind"} else "tsv"
    )
    return f"""
...
DB = BUNDLE / 'runs_colab_backfill.db'
TARGET = {spec.name!r}
LIMIT = {int(limit)}
TIMEOUT = {int(spec.worker_timeout_seconds)}
CANDIDATE = BUNDLE / ('candidates_%s.jsonl' % TARGET)
REPORT = BUNDLE / ('colab_%s_report.{report_ext}' % TARGET)
LOG = BUNDLE / ('colab_%s.log' % TARGET)
STATUS = BUNDLE / ('status_%s.json' % TARGET)
...
def read_report_counts():
    ...
    with REPORT.open('r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            processed += 1
            parts = line.rstrip('\\n').split('\\t')
            status = parts[status_idx] if status_idx >= 0 and status_idx < len(parts) else 'unknown'
            counts[status] += 1
    return processed, counts


def write_status(state, returncode=None, latest_line=''):
    processed, report_counts = read_report_counts()
    payload = {{
        'probe': TARGET,
        'state': state,
        'updated_at': utc_now(),
        ...
        'candidate_rows': count_candidate_rows(),
        'report_rows': processed,
        'report_status_counts': dict(report_counts),
        'db': str(DB),
        'report': str(REPORT),
        'log': str(LOG),
    }}
...
def run_streamed(cmd):
    ...
    with LOG.open('a', encoding='utf-8') as log:
        ...
        log.write(' '.join(cmd) + '\\n')
        log.flush()
        write_status('running', latest_line='starting subprocess')
        ...
        write_status(state, returncode=rc, latest_line=latest)
        log.write('=== %s end %s rc=%s ===\\n' % (TARGET, utc_now(), rc))
        log.flush()
        return rc


write_status('setup', latest_line='extracting source')
...
write_status('setup', latest_line='installing Python dependencies')
...
{command.strip()}
print(' '.join(cmd))
raise SystemExit(run_streamed(cmd))
""".strip()


def _colab_notebook_json(spec: ProbeSpec, limit: int) -> dict[str, Any]:
    code = _colab_code_for_probe(spec, limit)
    return {
        "cells": [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    f"# Cheap Probe Backfill: {spec.name}\n",
                    "\n",
                    f"Runs only `{spec.name}` against the Drive DB snapshot. "
                    "After it finishes, let Drive sync and run the matching local merge command from the README.\n",
                ],
            },
            {
                "cell_type": "code",
