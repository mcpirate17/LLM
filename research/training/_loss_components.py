...
PROB_COMPONENTS = frozenset()
_LOSS_NATIVE = None


def _loss_native_ext():
    global _LOSS_NATIVE
    if _LOSS_NATIVE is None:
        _LOSS_NATIVE = load_loss_native()
    return _LOSS_NATIVE
...
def loss_cross_entropy(flat_logits, flat_targets, log_probs, probs):
    return F.cross_entropy(flat_logits, flat_targets)


def loss_label_smoothed_ce(flat_logits, flat_targets, log_probs, probs):
...
def loss_rank_weighted_ce(flat_logits, flat_targets, log_probs, probs):
    if log_probs is None:
        log_probs = F.log_softmax(flat_logits, dim=-1)
    return _loss_native_ext().rank_weighted_ce(flat_logits, flat_targets, log_probs)


def loss_tropical_ce(flat_logits, flat_targets, log_probs, probs):
    if flat_logits.shape[-1] <= 1:
        return flat_logits.new_zeros(())
    if log_probs is None:
        log_probs = F.log_softmax(flat_logits, dim=-1)
    return _loss_native_ext().tropical_ce(flat_targets, log_probs)


def loss_contrastive_push(flat_logits, flat_targets, log_probs, probs):
...
def loss_entropy_reg(flat_logits, flat_targets, log_probs, probs):
    if log_probs is None:
        log_probs = F.log_softmax(flat_logits, dim=-1)
    return _loss_native_ext().entropy_reg(log_probs)


def loss_gradient_penalty(flat_logits, flat_targets, log_probs, probs):
    return flat_logits.pow(2).mean() * 0.001


def loss_kl_uniform(flat_logits, flat_targets, log_probs, probs):
    if log_probs is None:
        log_probs = F.log_softmax(flat_logits, dim=-1)
