...
const textMetric = (v) => {
  ...
  const text = String(v);
  return (
    <span title={text} style={{ fontSize: 10, color: "var(--text-secondary)" }}>
      {text}
    </span>
  );
};
...
function languageControlTierView(entry, tier) {
  ...
  const hasPoints =
    hasRawData && (points > 0 || tier.pointKeys.some((key) => breakdown[key] != null));
  const fillRatio = hasPoints ? points / tier.maxPoints : displayScore;
...
function LanguageControlLadder({ entry }) {
  ...
  return (
    <div
      title={[
        entry?.language_control_metric_version
          ? `version ${entry.language_control_metric_version}`
          : null,
        total != null ? `v14 controlled-lang points ${total.toFixed(1)}` : null,
        ...rows.map(
          (row) =>
            `${row.label}: SA ${fmt(row.sa, 2)}, NB order ${fmt(row.order, 2)}, NB score ${fmt(row.nb, 2)}${row.points != null ? `, pts ${row.points.toFixed(1)}/${row.maxPoints}` : ""}`,
        ),
      ...
      {rows.map((row) => {
        ...
        return (
          <div
            ...
            style={{
              display: "grid",
              gridTemplateColumns: "24px 48px 30px",
              alignItems: "center",
              gap: 4,
              marginBottom: 2,
            }}
          >
            <span
              style={{
                fontSize: 9,
                color:
                  row.key === "inv" && invSaFlag ? "var(--accent-yellow)" : "var(--text-muted)",
                fontWeight: 700,
              }}
            >
              {row.label}
            ...
            >
              <span
                style={{
                  display: "block",
                  height: "100%",
                  width: `${Math.max(3, row.fillRatio * 100)}%`,
                  background: muted ? "var(--text-muted)" : color,
                }}
              />
            </span>
            <span
              style={{ fontSize: 10, color, fontVariantNumeric: "tabular-nums", fontWeight: 700 }}
            >
              {row.displayScore == null ? "--" : row.displayScore.toFixed(2)}
              {row.key === "inv" && invSaFlag && (
                <span
                  role="img"
                  style={{ marginLeft: 3, color: "var(--accent-yellow)" }}
                  aria-label="INV SA below 0.85"
                >
                  !
                </span>
              )}
...
const RENDERERS = {
  ...
  _capability_quality: (entry) => {
    ...
    const color =
      status === "qualified" || status === "breakthrough"
        ? "var(--accent-green)"
        : status === "pending"
          ? "var(--accent-purple)"
  ...
  _verified: (entry) => {
    ...
    return (
      <span style={{ fontSize: 12, color: quality.color, fontWeight: 700 }} title={title}>
        {icon}
      </span>
    );
  },
  ...
  _gap: (entry) => {
    ...
    const gColor =
      gap < 0 ? "var(--accent-green)" : gap < 0.1 ? "var(--accent-yellow)" : "var(--accent-red)";
    return (
      <span style={{ fontSize: 11, color: gColor, fontWeight: 600 }}>
        {gap > 0 ? "+" : ""}
        {gap.toFixed(2)}
      </span>
    );
    ...
    <>
      ...
      <span
        style={{
          fontSize: 10,
          color:
            entry.model_source === "morphological_box"
              ? "var(--accent-purple)"
              : "var(--accent-blue)",
        }}
      >
  ...
  architecture_family: (entry) => {
    const fam = entry.architecture_family || "--";
    const mt = entry.lit_match_type;
    if (!mt) return fam;
    // Literature provenance badge: green=exact, blue=family, yellow=partial,
    // purple=novel (project original). Source: literature_attribution table.
    const colors = {
    ...
    const color = colors[mt] || "var(--text-muted)";
    const title = entry.lit_family
      ? `${entry.lit_family} (${mt})${entry.lit_ref ? " — " + entry.lit_ref : ""}`
      : mt;
    return (
  ...
  _composition: (entry) => {
    ...
    return (
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        ...
        {templates.length > 3 && (
          <span style={{ fontSize: 9, color: "var(--text-muted)" }}>+{templates.length - 3}</span>
        )}
      </div>
    );
  },
  ...
  _vs_reference: (entry) =>
    entry._vs_reference != null ? `${entry._vs_reference.toFixed(0)}%` : "--",
  ...
  composite_score: (entry) => {
    ...
    return (
      <span
        style={{
          color: Number.isFinite(score) ? scoreColor(score) : "var(--text-muted)",
          fontWeight: 700,
        }}
      >
        {fmt(entry.composite_score, 3)}
        <span
          style={{ fontSize: 10, marginLeft: 4, color: quality.color }}
          title={`${quality.label}: ${quality.version}`}
        >
          {tokIcon}
        </span>
      </span>
  ...
  champion_tiny_model_score: (e) =>
    coloredMetric(
      e.champion_tiny_model_score,
      [
        [null, 20, "var(--accent-red)"],
        [35, null, "var(--accent-green)"],
        [20, 35, "var(--accent-yellow)"],
      ],
      1,
    ),
  ...
  fp_jacobian_erf_density: (e) =>
    coloredMetric(
      e.fp_jacobian_erf_density,
      [
        [null, 0.08, "var(--text-muted)"],
        [0.18, null, "var(--accent-green)"],
        [0.08, 0.18, "var(--accent-yellow)"],
      ],
      3,
    ),
  fp_id_collapse_rate: (e) =>
    coloredMetric(
      e.fp_id_collapse_rate,
      [
        [null, 0.02, "var(--text-muted)"],
        [0.1, null, "var(--accent-green)"],
        [0.02, 0.1, "var(--accent-yellow)"],
      ],
      3,
    ),
  fp_id_collapse_rate_normalized: (e) =>
    coloredMetric(
      e.fp_id_collapse_rate_normalized,
      [
        [null, 0.02, "var(--text-muted)"],
        [0.1, null, "var(--accent-green)"],
        [0.02, 0.1, "var(--accent-yellow)"],
  ...
  fp_jacobian_erf_decay_slope: (e) =>
    coloredMetric(
      e.fp_jacobian_erf_decay_slope,
      [
        [null, 0.0, "var(--text-muted)"],
        [0.2, null, "var(--accent-green)"],
        [0.0, 0.2, "var(--accent-yellow)"],
      ],
      3,
    ),
  ...
  fp_icld_velocity: (e) => (
    <span style={{ color: "var(--text-muted)" }}>{fmt(e.fp_icld_velocity, 3)}</span>
  ),
  fp_icld_delta_loss: (e) => (
    <span style={{ color: "var(--text-muted)" }}>{fmt(e.fp_icld_delta_loss, 3)}</span>
  ),
  max_viable_seq_len: (e) =>
    e.max_viable_seq_len != null ? Number(e.max_viable_seq_len).toFixed(0) : "--",
  quant_int8_retention: (e) =>
    e._quant_retention_pct != null ? `${e._quant_retention_pct.toFixed(1)}%` : "--",
  ...
  arch_quality_score: (e) => (
    <span
      style={{
        color:
          e.arch_quality_score > 0.7
            ? "var(--accent-green)"
            : e.arch_quality_score < 0.4
              ? "var(--accent-red)"
              : "var(--text-primary)",
      }}
    >
  ...
  investigation_robustness: (e) => (
    <span
      style={{
        color: e.investigation_robustness >= 0.5 ? "var(--accent-green)" : "var(--accent-red)",
      }}
    >
  ...
  peak_ppl: (e) => (
    <span style={{ color: "var(--accent-cyan)", fontWeight: 600 }}>{fmt(e.peak_ppl, 2)}</span>
  ),
  validation_baseline_ratio: (e) => (
    <span
      style={{
        color: e.validation_baseline_ratio < 1 ? "var(--accent-green)" : "var(--accent-red)",
      }}
    >
      {fmt(e.validation_baseline_ratio)}
    </span>
  ),
  ...
  hellaswag_acc: (e) => {
    ...
    return (
      <span style={{ color: hellaswagColor(e.hellaswag_acc), fontWeight: 600 }}>
        {(Number(e.hellaswag_acc) * 100).toFixed(1)}%
      </span>
    );
  },
  induction_screening_auc: (e) => (
    <span style={{ color: probeAucColor(e.induction_screening_auc), fontWeight: 600 }}>
      {fmt(e.induction_screening_auc, 3)}
    </span>
  ),
  induction_intermediate_auc: (e) => (
    <span style={{ color: probeAucColor(e.induction_intermediate_auc), fontWeight: 600 }}>
      {fmt(e.induction_intermediate_auc, 3)}
    </span>
  ),
  induction_validation_auc: (e) => (
    <span style={{ color: probeAucColor(e.induction_validation_auc), fontWeight: 600 }}>
      {fmt(e.induction_validation_auc, 3)}
    </span>
  ),
  ...
  induction_validation_gap_accuracies_json: (e) =>
    jsonMetric(e.induction_validation_gap_accuracies_json),
  ...
  ar_legacy_auc: (e) => (
    <span style={{ color: probeAucColor(e.ar_legacy_auc), fontWeight: 600 }}>
      {fmt(e.ar_legacy_auc, 3)}
    </span>
  ),
  ...
  ar_curriculum_auc_pair_final: (e) => (
    <span style={{ color: probeAucColor(e.ar_curriculum_auc_pair_final), fontWeight: 600 }}>
      {fmt(e.ar_curriculum_auc_pair_final, 3)}
    </span>
  ),
  ar_curriculum_s0_retention: (e) => {
    ...
    const color =
      v < 0.3 ? "var(--accent-red)" : v < 0.7 ? "var(--accent-yellow)" : "var(--accent-green)";
    return (
      <span
  ...
  ar_curriculum_max_passing_stage: (e) => {
    ...
    const n = Number(v);
    const color =
      n < 0 ? "var(--accent-red)" : n < 3 ? "var(--accent-yellow)" : "var(--accent-green)";
  ...
  binding_screening_auc: (e) => (
    <span style={{ color: probeAucColor(e.binding_screening_auc), fontWeight: 600 }}>
      {fmt(e.binding_screening_auc, 3)}
    </span>
  ),
  binding_intermediate_auc: (e) => (
    <span style={{ color: probeAucColor(e.binding_intermediate_auc), fontWeight: 600 }}>
      {fmt(e.binding_intermediate_auc, 3)}
    </span>
  ),
  ...
  blimp_overall_accuracy: (e) => {
    ...
    return (
      <span style={{ color: blimpColor(e.blimp_overall_accuracy), fontWeight: 600 }}>
        {(Number(e.blimp_overall_accuracy) * 100).toFixed(1)}%
      </span>
    );
  },
  ...
  pre_inv_score: (e) => {
    ...
    const pisColor =
      pis >= 50 ? "var(--accent-green)" : pis >= 20 ? "var(--accent-yellow)" : "var(--accent-red)";
    return <span style={{ color: pisColor, fontWeight: 600 }}>{fmt(pis, 1)}</span>;
  },

  wikitext_ppl_trajectory: (e) => {
    const data = Array.isArray(e.wikitext_ppl_trajectory)
      ? e.wikitext_ppl_trajectory
      : typeof e.wikitext_ppl_trajectory === "string"
        ? e.wikitext_ppl_trajectory.split(",").map((v) => parseFloat(v.trim()))
        : null;
  ...
  evaluation_stage: (e) => (
    <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
      ...
      {e.is_frontier_signal && (
        <StatusBadge
          type="FRONTIER_SIGNAL"
          label="FRONTIER"
          title="Model beats reference PPL at equal budget"
        />
      )}
      {(e.improvement_ratio > 2.0 || e.is_slow_burn) && (
        <StatusBadge
          type="SLOW_BURN"
          label="SLOW-BURN"
          title="Sharp trajectory: PPL improved >2x"
        />
      )}
      {e.divergence_step && (
        <StatusBadge
          type="DIVERGED"
          label={`DIVERGED @ ${e.divergence_step}`}
          title="PPL diverged >2x peak"
        />
      )}
      {!e.divergence_step && e.wikitext_ppl_trajectory?.length >= 4 && (
        <StatusBadge
          type="STABLE_GENERALIZER"
          label="STABLE"
          title="No divergence seen in 4000 steps"
        />
      )}
    </div>
  ),
  ...
  robustness_grade: (e) => {
    const grade =
      e.robustness_grade ||
      (e.investigation_robustness >= 0.8
        ? "A"
        : e.investigation_robustness >= 0.5
          ? "B"
          : e.investigation_robustness != null
            ? "C"
            : null);
    const statusType =
      grade === "A" ? "ROBUST" : grade === "B" ? "STABLE" : grade === "C" ? "FRAGILE" : null;
    return statusType ? (
      <StatusBadge type={statusType} label={grade} title={`Robustness Grade ${grade}`} />
    ) : null;
  },
  ...
  _compression_ratio: (e, ctx) => (
    <>
      <div style={{ fontSize: 11 }}>
        {ctx.compression?.ratio != null ? `${(ctx.compression.ratio * 100).toFixed(0)}%` : "--"}
      </div>
      <div style={{ fontSize: 10, color: "var(--text-muted)" }}>
        {ctx.compression?.memoryMb?.toFixed(2)} MB · {ctx.compression?.label}
      </div>
    </>
  ),
  ...
  _metric_quality: (e, ctx) => (
    <>
      <div style={{ fontSize: 10, color: "var(--text-muted)" }}>
        Q: {ctx.reproPacket.label} · R: {ctx.reproPacket.label}
      </div>
      {ctx.isExpanded && (
          ...
          {ctx.chips.map((c) => (
            <span
              ...
              style={{
                ...
                padding: "1px 5px",
                borderRadius: 4,
                background: `${reliabilityColor(c.reliability)}22`,
                color: reliabilityColor(c.reliability),
              }}
            >
              {c.label}
            </span>
...
export { fmt, RENDERERS, TD_STYLE_OVERRIDES };
