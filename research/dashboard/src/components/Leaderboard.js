...
function Leaderboard({
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => {
    const baseline = ['_score', 'tier', 'architecture_family', 'composite_score', 'discovery_loss_ratio', 'validation_loss_ratio', 'screening_loss_ratio', 'screening_novelty', 'investigation_loss_ratio', 'validation_baseline_ratio', '_actions'];
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <div className="card-title">Qualified Models ({rawEntries.length})</div>
      <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
        Ranked candidates with tiered evidence. Click a row for details. 
        Updated: {lastUpdated?.toLocaleTimeString()}
      </p>

      {/* Tabs and Filters batch */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        {['all', 'screening', 'investigation', 'validation', 'breakthrough'].map(t => (
          <button key={t} onClick={() => setActiveTier(t)} className={`step-btn \${activeTier === t ? 'active' : ''}`} style={{ fontSize: 11, padding: '4px 10px' }}>
            {t === 'all' ? 'All' : TIER_LABELS[t]} {tierCounts[t] ? `(\${tierCounts[t]})` : ''}
          </button>
        ))}
        <button onClick={() => setShowColumnPicker(!showColumnPicker)} className="refresh-btn" style={{ fontSize: 11, marginLeft: 'auto' }}>Columns</button>
        <button onClick={fetchLeaderboard} className="refresh-btn" style={{ fontSize: 11 }}>Refresh</button>
      </div>

      <div style={{ marginBottom: 12 }}>
        <input 
          type="text" placeholder="Search models..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
          style={{ width: '100%', maxWidth: 400, padding: '6px 10px', fontSize: 12, border: '1px solid var(--border)', borderRadius: 4, background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
        />
      </div>
          ...
          <thead style={{ position: 'sticky', top: 0, zIndex: 2, background: 'var(--bg-secondary)' }}>
            <tr>
              ...
              {COLUMNS.filter(c => visibleColumns.includes(c.key)).map(c => (
                <th key={c.key} style={thStyle} onClick={() => handleSort(c.key)}>{c.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
