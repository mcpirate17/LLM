"""Natural-language corpus for the champion Small-AR probe.
...
SMALL_AR_STORY_CORPUS_VERSION = "small_ar_story_corpus_v1"
...
@dataclass(frozen=True, slots=True)
class SmallARStoryCorpusConfig:
    seed: int = 0
...
def _key_space(cfg: SmallARStoryCorpusConfig) -> list[StoryKey]:
...
def _value_space(cfg: SmallARStoryCorpusConfig) -> list[str]:
...
def build_small_ar_story_corpus(
    cfg: SmallARStoryCorpusConfig | None = None,
) -> StoryCorpusSpec:
    """Build deterministic train/eval stories for Small-AR v3 calibration."""
    cfg = cfg or SmallARStoryCorpusConfig()
    ...
    return StoryCorpusSpec(
        version=SMALL_AR_STORY_CORPUS_VERSION,
        seed=int(cfg.seed),
