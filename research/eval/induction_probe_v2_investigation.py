"""Investigation-tier induction probe (v2, 2026-04-17, native-accelerated 2026-04-19).
...
Differences from the screening-tier probe:
  * Training cycles through the eval gap set instead of fixing gap=8. The
    ...
    `PROBE_CALIBRATION_2026-04-17.md` for empirical measurements.
  * Only runs at investigation tier (post-screening).
  * Writes to dedicated columns (`induction_v2_investigation_*`).

Performance notes (2026-04-19 rewrite):
...
def _run_induction_v2_single_seed(
    ...
    *,
    ...
    should use :func:`run_induction_v2_investigation` which takes the
