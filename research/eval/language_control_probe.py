"""Unified language-control probe — one training pass, two evaluations.
...
LANGUAGE_CONTROL_METRIC_VERSION = "language_control_v2"
...
class LanguageControlResult:
    ...
    metric_version: str = LANGUAGE_CONTROL_METRIC_VERSION

    def to_dict(self) -> Dict[str, Any]:
        out = {
            "language_control_metric_version": self.metric_version,
            "language_control_train_steps": self.n_train_steps,
            "language_control_active_vocab_size": self.active_vocab_size,
            "language_control_elapsed_ms": self.elapsed_ms,
            "language_control_status": self.status,
        }
        if self.checkpoints:
            out["language_control_checkpoints"] = list(self.checkpoints)
        out.update(self.nano_blimp)
        out.update(self.synthetic_association)
        return out


def language_control_probe(
    model: nn.Module,
    *,
    ...
    """Train once on the language-control association corpus, then
