...
def run_selective_canary_latency_benchmark(
    ...
    def _run_mode(mode: str, *, layer_exec: bool) -> Dict[str, Any]:
        ...
        for idx in range(iterations):
            ...
            model = compile_model_native_first(
                fake_graphs, vocab_size=256, max_seq_len=16
            )
            ...
            execution_paths[execution_path] = (
                int(execution_paths.get(execution_path) or 0) + 1
            )

            layer_build = (report.get("selective_execution") or {}).get(
                "layer_build"
            ) or {}
        ...
        avg_applied_layers = (
            sum(applied_layers) / float(len(applied_layers)) if applied_layers else 0.0
        )
        return {
    ...
    with (
        ...
        patch(
            ...
            side_effect=lambda *args, **kwargs: _DummyModel(),
        ),
        patch(
            "research.scientist.native_runner_adapter.build_designer_layer_modules",
            return_value=replacement_payload,
        ),
        patch(
            "research.scientist.native.compiler._validate_designer_layer_adapter_contract",
            return_value=None,
        ),
    ):
