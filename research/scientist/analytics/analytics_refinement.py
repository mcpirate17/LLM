...
class RefinementAnalyzer:
    ...
    def analyze_program_for_refinement(
        ...
        qualified_rates = [pop_s1_rates[op] for op, n in pop_n_used.items() if n >= 5]
        mean_s1_rate = (
            sum(qualified_rates) / len(qualified_rates) if qualified_rates else 0.0
        )

        n_programs_total = (
            sum(pop_n_used.values()) // max(1, len(pop_n_used)) if pop_n_used else 0
        )
        n_stage1_passed = (
            sum(int(r.get("n_stage1_passed") or 0) for r in op_stats_rows)
            // max(1, len(op_stats_rows))
            if op_stats_rows
            else 0
        )
        ...
        analysis_quality = (
            "full" if qualified_rates else ("partial" if op_stats_rows else "no_data")
        )
    ...
    def _build_recipe(
        ...
        risky_ops = [
            h for h in op_health if h["health"] == "risky" and h["n_used"] >= 8
        ]
        weak_ops = [h for h in op_health if h["health"] == "weak"]
        significant_gaps = [
            g for g in behavioral_gaps if g["severity"] in ("medium", "high")
        ]
        ...
        param_count = program_row.get("param_count") or program_row.get(
            "graph_n_params_estimate"
        )
        has_reasonable_loss = (
            isinstance(loss_ratio, (int, float)) and float(loss_ratio) < 1.1
        )
        has_high_params = (
            isinstance(param_count, (int, float)) and float(param_count) > 100000
        )

        if risky_ops:
            recommended_intent = "quality"
            primary_target = (
                f"Replace {len(risky_ops)} risky op(s) with proven alternatives"
            )
            ...
            primary_target = (
                f"Address {top_gap['label']} gap (z={top_gap['z_score']:+.1f})"
            )
    ...
    def strategy_backtest(self) -> Dict[str, Any]:
        ...
        for row in rows:
            ...
            if not intent or intent not in (
                ...
                if config.get("mode") == "novelty":
                    intent = "novelty"
                elif config.get("mode") == "scale_up":
                    intent = "quality"
                else:
                    intent = "balanced"
            ...
            results.append(
                {
                    ...
                    "s1_pass_rate": d["n_s1_passed"] / d["n_programs"]
                    if d["n_programs"] > 0
                    else 0.0,
                    "stable_pass_rate": d["n_stable_passed"] / d["n_programs"]
                    if d["n_programs"] > 0
                    else 0.0,
                    "avg_best_loss": d["total_loss"] / d["loss_count"]
                    if d["loss_count"] > 0
                    else None,
                    "avg_best_novelty": d["total_novelty"] / d["novelty_count"]
                    if d["novelty_count"] > 0
                    else None,
                    "avg_throughput": d["total_throughput"] / d["throughput_count"]
                    if d["throughput_count"] > 0
                    else None,
                    "avg_duration": d["total_duration"] / d["n_experiments"]
                    if d["n_experiments"] > 0
                    else 0.0,
                }
            )
