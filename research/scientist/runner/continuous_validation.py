...
class _ContinuousValidationMixin:
    ...
    def _run_scaling_reference_compare(
        ...
        *,
        ...
        comparison = self._get_scaling_reference_manager().compare_candidate(
            ...
            batch_size=max(
                1, min(4, int(getattr(config, "validation_batch_size", 4) or 4))
            ),
    ...
    def _make_validation_input_batches(
        ...
        seq_len = min(128, int(getattr(config, "validation_seq_len", 128) or 128))
        batch_size = max(
            1, min(4, int(getattr(config, "validation_batch_size", 2) or 2))
        )
        return [
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                result["tinystories_perplexity"] = ts_result.get(
            ...
            try:
                long_context = run_long_context_sweep(
                    ...
                    or max(float(val_loss_ratio or 1.0), 1e-6),
                    seq_lens=(512, 1024),
                    n_steps=min(60, max(20, int(config.validation_steps) // 100)),
                    batch_size=max(
                        1, min(2, int(getattr(config, "validation_batch_size", 2) or 2))
                    ),
                    lr=float(best_seed.get("optimizer_lr") or config.stage1_lr)
                    if best_seed
                    else float(config.stage1_lr),
                )
            ...
            try:
                ...
                result["activation_sparsity_score"] = sparsity_result.get(
            ...
            try:
                ...
                if quant_result:
                    ...
                    result["quant_quality_per_byte"] = quant_result.get(
                        "quality_per_byte"
                    )
            except Exception as exc:
                logger.debug(
            ...
            if scaling_enabled:
                try:
                    ...
                    result["scaling_flop_efficiency"] = wall_result.get(
                        "time_scaling_factor"
                    )
                except Exception as exc:
                    logger.debug(
        ...
        if scaling_enabled:
            try:
                ...
                if scaling_payload is not None:
                    ...
                    result["scaling_param_efficiency"] = scaling_payload.get(
                        "best_param_efficiency"
                    )
                    result["scaling_flop_efficiency"] = scaling_payload.get(
                        "flop_efficiency"
                    )
                    result["scaling_best_family"] = scaling_payload.get(
                        "best_param_efficiency_family"
                    )
                    scaling_gate_passed = bool(
                        scaling_payload.get("scaling_gate_passed")
                    )
                    result["scaling_confidence"] = str(
            ...
            if bool(getattr(config, "scaling_d512_enabled", True)):
                try:
                    ...
                    if d512_payload is not None:
                        result["scaling_d512_param_efficiency"] = d512_payload.get(
                            "best_param_efficiency"
                        )
        ...
        raw_breakthrough_passed = val_loss_ratio is not None and float(
            val_loss_ratio
        ) <= float(getattr(config, "breakthrough_raw_threshold", 0.70) or 0.70)
        normalized_breakthrough_passed = val_normalized_ratio is not None and float(
            val_normalized_ratio
        ) >= float(getattr(config, "breakthrough_normalized_threshold", 0.85) or 0.85)
        ...
        seeds_passed_count = len(passed_seeds) if passed_seeds else 0
        seeds_total = int(getattr(config, "validation_n_seeds", 5) or 5)
        seeds_can_promote = (seeds_total < 3) or (seeds_passed_count >= 1)
        if (
        ...
        elif seeds_passed_count == 0 and seeds_total >= 3:
                ...
                if v_loss is not None:
                    try:
                        ...
                        v_baseline_ratio = baseline.compare(
                            ...
                            d_model=config.model_dim,
                            seq_len=min(
                                128, int(getattr(config, "validation_seq_len", 128))
                            ),
                            n_steps=max(1, baseline_steps),
                            vocab_size=config.vocab_size,
                ...
                try:
                    if graph_json_str and len(passed_seeds) > 0:
                        ...
                        from ..native_runner import (
                            compile_model_native_first as _compile_model,
                        )
