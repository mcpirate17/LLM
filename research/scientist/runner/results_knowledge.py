...
from ..llm.knowledge_utils import (
    canonical_knowledge_text,
    is_extracted_knowledge_low_value,
    ...
    def _maybe_extract_knowledge(
        ...
        try:
            ...
            for row in existing_entries:
                ...
                if not eid:
                    continue
                existing_by_title[canonical_knowledge_text(row.get("title") or "")] = (
                    eid
                )
                existing_by_content[
            ...
            for entry in entries:
                ...
                if is_extracted_knowledge_low_value(raw_title, raw_content):
                    skipped_low_value += 1
                    continue
                ...
                title_key = canonical_knowledge_text(title)
                content_key = canonical_knowledge_text(content)

                existing_entry_id = existing_by_title.get(
                    title_key
                ) or existing_by_content.get(content_key)
                if not existing_entry_id:
                    semantic_tokens = knowledge_tokens(f"{title} {content}")
                    for eid, seen_tokens in existing_by_semantic.get(category, []):
                        if is_semantic_duplicate(semantic_tokens, seen_tokens):
