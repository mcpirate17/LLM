...
def _freeze_op_pair_priors(priors: List[Dict[str, Any]]) -> Tuple[Tuple[str, float], ...]:
    return tuple(
        (
...
def _cached_signal_weight_maps(
    ...
    for bucket_name, n_graphs, s1_rate, top_ops in fingerprint_buckets:
        ...
        for template_name, boost in _BUCKET_TEMPLATE_BOOSTS.get(bucket_name, {}).items():
            if template_name in TEMPLATES:
                template_bonuses[template_name] = template_bonuses.get(template_name, 0.0) + (boost * dominance)
    ...
    template_weights = {
        template_name: round(DEFAULT_TEMPLATE_WEIGHTS.get(template_name, 1.0) + bonus, 4)
...
def _judgment_rerank(
    ...
    for graph in graphs:
        ...
        if result.risk_flags:
            log.info("judgment_rerank: discarding candidate with risk flags: %s",
                     result.risk_flags)
            skipped += 1
            continue
    ...
    def _execute_experiment(self, exp_id: str, config: RunConfig,
        ...
        if _eff_weight >= 0.3:  # only apply when efficiency is prioritized
            ...
            for _cat in ("structural", "parameterized"):
                ...
                grammar.category_weights[_cat] = round(
                    min(8.0, _base * _eff_boost), 2
                )
            # Boost specific efficiency-related ops
            for _op in ("moe_2expert", "moe_topk", "block_sparse_linear",
                         "bottleneck_proj", "linear_proj_down", "selective_scan"):
