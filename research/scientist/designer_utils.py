...
def workflow_to_computation_graph(
    workflow_json: Dict[str, Any], default_model_dim: int = 256
) -> ComputationGraph:
...
def run_designer_graph(
    workflow_json: Dict[str, Any], device: str = "cpu"
) -> Dict[str, Any]:
...
def get_designer_components() -> List[Dict[str, Any]]:
    ...
    components = [
        ...
        },
        {
            ...
            "name": "Output Head",
            "category": "io",
            "description": "Graph output (B, S, D).",
    ...
    for name, op in PRIMITIVE_REGISTRY.items():
        ...
        inputs = (
            ...
            if op.n_inputs == 1
                ...
                [{"name": "a", "dtype": "tensor"}, {"name": "b", "dtype": "tensor"}]
                if op.n_inputs == 2
                else []
            )
        )
        ...
        for key in op.config_keys:
            p_type = (
                "float"
                if any(k in key for k in ["prob", "scale", "damping"])
                else ("enum" if "operator" in key else "integer")
            )
            params[key] = {
                "type": p_type,
                "default": None,
