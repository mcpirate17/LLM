...
def _score_for_nb(value: float) -> float:
    result = compute_composite_v14(
        decompose=True,
        tier="validation",
        controlled_lang_s10_nb_score=value,
    )
    return result["breakdown"]["cl_s10_nb_bucket"]


def test_controlled_lang_s10_nb_bucket_points() -> None:
...
def test_controlled_lang_inv_nb_bucket_points() -> None:
    result = compute_composite_v14(
        decompose=True,
        tier="validation",
        controlled_lang_inv_nb_score=0.91,
    )

    assert result["breakdown"]["cl_inv_nb_bucket"] == 18.75
