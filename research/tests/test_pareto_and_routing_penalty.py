...
class TestCompositeScoreRoutingPenalty(unittest.TestCase):
    ...
    def test_good_routing_gets_bonus(self):
        ...
        with_routing = self.score(
            screening_lr=0.5, screening_nov=0.8, routing_savings=0.3
        )
    ...
    def test_wasteful_routing_penalized(self):
        ...
        penalized = self.score(
            screening_lr=0.98, screening_nov=0.5, routing_savings=0.01
        )
    ...
    def test_wasteful_routing_good_loss_no_penalty(self):
        ...
        with_routing = self.score(
            screening_lr=0.3, screening_nov=0.8, routing_savings=0.01
        )
    ...
    def test_penalty_scales_with_waste(self):
        ...
        p1 = self.score(
            screening_lr=0.98, screening_nov=0.5, routing_savings=0.04
        )  # close to threshold
        p2 = self.score(
            screening_lr=0.98, screening_nov=0.5, routing_savings=0.00
        )  # maximally wasteful
    ...
    def test_penalty_uses_best_available_loss(self):
        ...
        s = self.score(
            screening_lr=0.99, screening_nov=0.5, val_baseline=0.5, routing_savings=0.0
        )
        base = self.score(screening_lr=0.99, screening_nov=0.5, val_baseline=0.5)
