...
def is_causal_op(op_name: str) -> bool:
    ...
    try:
        ...
        diff = (
            torch.abs(out_base[:, :midpoint, ...] - out_mod[:, :midpoint, ...])
            .max()
            .item()
        )
...
def test_op_causality_regression(op_name):
    ...
    assert is_causal_op(op_name), (
        f"Operation '{op_name}' violates causality! This will cause reward hacking in Stage 1."
    )
