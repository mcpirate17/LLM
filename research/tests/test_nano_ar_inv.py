"""Smoke + determinism tests for nano-AR-INV (CPU only)."""
...
def test_query_prompt_format():
    from research.eval.nano_ar_inv_corpus import Fact, query_prompt
...
def test_probe_smoke_on_tiny_graph():
    ...
    result = nano_ar_inv(graph_json=graph_to_json(g), device="cpu", cfg=cfg)
    assert result.status in ("ok", "timeout"), result.error
    assert 0.0 <= result.in_dist_pair_match_acc <= 1.0
