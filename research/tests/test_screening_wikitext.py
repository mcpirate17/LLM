...
class TestNonInvasiveEval:
    ...
    def test_model_params_unchanged(self, tiny_model):
        ...
        with patch(
            "research.eval.wikitext_eval._download_wikitext"
        ) as mock_dl:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                result = screening_wikitext_eval(
                    tiny_model, vocab_size=256, device="cpu",
                    seq_len=16, n_train_steps=5,
                    n_train_batches=2, n_eval_batches=2, batch_size=2,
                )
    ...
    def test_clone_failure_graceful(self, tiny_model):
        ...
        with patch("research.eval.wikitext_eval.copy.deepcopy", side_effect=RuntimeError("clone boom")):
            with patch("research.eval.wikitext_eval._prepare_batches") as mock_pb:
                mock_pb.return_value = ([torch.zeros(2, 16)], [torch.zeros(2, 16)], 100, 50)
                result = screening_wikitext_eval(
                    tiny_model, vocab_size=256, device="cpu",
                )
...
class TestMetadataFields:
    ...
    def test_budget_fields(self, tiny_model):
        ...
        for key in ("n_train_steps", "n_train_batches", "n_eval_batches",
                     "batch_size", "seq_len", "max_chars_train", "max_chars_val"):
...
class TestGracefulFailure:
    def test_data_download_failure(self, tiny_model):
        with patch("research.eval.wikitext_eval._prepare_batches", side_effect=OSError("no network")):
            result = screening_wikitext_eval(
                tiny_model, vocab_size=256, device="cpu",
            )
...
class TestBatchCache:
    ...
    def test_put_and_get(self):
        ...
        _put_cached_batches("wikitext-2-raw-v1", 256, 16, 4, 8, 100_000, "train", 42, batches)
        got = _get_cached_batches("wikitext-2-raw-v1", 256, 16, 4, 8, 100_000, "cpu", "train", 42)
    ...
    def test_different_seed_different_key(self):
        batches = [torch.randn(2, 16)]
        _put_cached_batches("wikitext-2-raw-v1", 256, 16, 4, 8, 100_000, "train", 42, batches)
        got = _get_cached_batches("wikitext-2-raw-v1", 256, 16, 4, 8, 100_000, "cpu", "train", 99)
        assert got is None
