...
def test_ir_roundtrip_equivalence():
    ...
    model_std = compile_model(
        [g], vocab_size=vocab_size, max_seq_len=seq_len, use_ir=False
    )
    model_std.eval()

    # Compile with IR backend
    model_ir = compile_model(
        [g], vocab_size=vocab_size, max_seq_len=seq_len, use_ir=True
    )
    model_ir.eval()
