...
def test_perf_contract_artifact_roundtrip(tmp_path):
    ...
    contract = build_perf_contract(
        ...
        budget_verdict={"passed": False, "checks": [{"metric": "duplicate_work.detected_count"}]},
        duplicate_work=duplicate_work,
    )

    artifact_path = emit_perf_artifact(contract, root=str(tmp_path), slug="exp_123")
    artifacts = list_recent_perf_artifacts(root=str(tmp_path), component="research", limit=5)
