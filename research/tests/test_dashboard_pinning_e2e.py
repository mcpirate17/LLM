...
def test_dashboard_pinning_api_and_sorting(tmp_path):
    ...
    entry1 = nb.upsert_leaderboard(result_id=res1, model_source="test", architecture_desc="Candidate 1", screening_loss_ratio=0.5)
    entry2 = nb.upsert_leaderboard(result_id=res2, model_source="test", architecture_desc="Candidate 2", screening_loss_ratio=0.4)
    nb.close()
    ...
    # Pin res1
    pin_resp = client.post("/api/leaderboard/pin", json={"entry_id": entry1, "pinned": True})
