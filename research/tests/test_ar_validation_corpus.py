...
def test_story_corpus_builds_natural_language_story_shape():
    ...
    spec = build_ar_validation_story_corpus(_tiny_config())
    story = spec.train_stories[0]

    assert spec.version == AR_VALIDATION_STORY_CORPUS_VERSION
...
def test_held_key_split_uses_keys_absent_from_training_stories():
    from research.eval.ar_validation_corpus import build_ar_validation_story_corpus
...
def test_binary_choices_keep_answer_and_one_in_episode_distractor():
    ...
    spec = build_ar_validation_story_corpus(
        ARValidationStoryCorpusConfig(
            seed=11,
            n_train_stories=2,
            n_in_dist_eval_stories=1,
...
def test_default_value_pool_stays_distinct_under_gpt2_checkpoint_vocab():
    ...
    values = _value_space(ARValidationStoryCorpusConfig(n_values=32))
    suffixes = {
