...
def test_archive_is_quarantined_outside_research_package() -> None:
    ...
    assert not active_archive.exists(), f"Archive scripts leaked back into active package: {active_archive}"
    assert quarantined_archive.is_file(), f"Missing archive quarantine README: {quarantined_archive}"
...
def test_generated_runtime_artifacts_are_not_present_in_research_tree() -> None:
    forbidden_paths = [
        ...
        RESEARCH_ROOT / "runtime" / "native" / "cython" / "aria_bridge.cpython-312-x86_64-linux-gnu.so",
    ]
    present = [str(path) for path in forbidden_paths if path.exists()]
    assert not present, f"Generated runtime artifacts should not live in research/: {present}"
