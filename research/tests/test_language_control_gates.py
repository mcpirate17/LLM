...
from research.tools.apply_language_control_gates import (
    _candidate_rows,
...
def test_s05_nb_screening_failure_threshold_is_conservative() -> None:
    ...
    assert is_language_control_nb_screening_failure(0.6499)
    assert not is_language_control_nb_screening_failure(0.65)


def test_s05_nb_gate_blocks_advanced_tiers_until_passed() -> None:
    assert not allows_language_control_advanced_tiers(None)
    assert not allows_language_control_advanced_tiers(0.6499)
    assert allows_language_control_advanced_tiers(0.65)
...
def test_s05_sa_gate_blocks_advanced_tiers_without_escape() -> None:
    assert not allows_language_control_advanced_tiers(
        0.80,
        sa_score=0.30,
        erf_density=0.015625,
        erf_decay_slope=-0.04,
    ...
    assert allows_language_control_advanced_tiers(
        0.80,
        sa_score=0.30,
        erf_density=0.0625,
        erf_decay_slope=-0.103282,
...
def test_apply_s05_nb_failure_screens_out_candidate() -> None:
    ...
    assert details["reason"] == "language_control_s05_nb_below_threshold"
...
def test_apply_s05_sa_failure_screens_out_candidate_without_escape() -> None:
    ...
    assert lb == (
        "screened_out",
        0,
        "language_control_s05_sa: score 0.4000 below screening threshold 0.65 "
    ...
    details = json.loads(pr[1])
    assert details["reason"] == "language_control_s05_sa_below_threshold_without_escape"
    assert details["language_control_s05_sentence_assoc_score"] == 0.40
...
def test_manual_override_preserves_known_s05_sa_candidate() -> None:
    ...
    assert override["reason"] == "manual_pass_harder_ar_gate_good"
...
def test_apply_s10_nb_failure_screens_out_candidate() -> None:
    ...
    details = json.loads(pr[1])
    assert details["reason"] == "language_control_s10_nb_below_threshold"
    assert details["stage"] == "s1.0"
    assert details["language_control_s10_binding_score"] == 0.62
...
def test_apply_inv_nb_failure_screens_out_candidate() -> None:
    ...
    details = json.loads(pr[1])
    assert details["reason"] == "language_control_investigation_binding_below_threshold"
    assert details["stage"] == "investigation"
    assert details["language_control_investigation_binding_score"] == 0.64
...
def test_apply_s10_nb_sa_failure_screens_out_candidate() -> None:
    ...
    details = json.loads(pr[1])
    assert details["reason"] == "language_control_s10_nb_sa_below_threshold"
    assert details["language_control_s10_binding_score"] == 0.75
    assert details["language_control_s10_sentence_assoc_score"] == 0.25
