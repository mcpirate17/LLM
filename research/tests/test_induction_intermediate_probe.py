...
def test_result_to_dict_has_all_keys():
    ...
    d = r.to_dict()
    assert "induction_intermediate_auc" in d
    assert "induction_intermediate_max_gap_acc" in d
    assert "induction_intermediate_protocol_version" in d


def test_multi_seed_probe_restores_model_state_between_runs(monkeypatch):
    from research.eval import induction_intermediate_probe as probe
    ...
    monkeypatch.setattr(probe, "_run_induction_intermediate_on", _fake_run)

    result = probe.run_induction_intermediate(
        _TinyModel(),
        seeds=(1, 2, 3),
        device="cpu",
    )
...
def test_attention_2l_beats_conv3_2l():
    ...
    r_attn = run_induction_intermediate(attn, n_train_steps=500, n_eval=100, device=dev)
    r_conv = run_induction_intermediate(conv, n_train_steps=500, n_eval=100, device=dev)
