...
class TestPrompts(unittest.TestCase):
    ...
    def test_all_prompts_importable(self):
        ...
        for name, prompt in [
            ...
            self.assertIn(
                "{context}", prompt, f"{name}_PROMPT missing {{context}} placeholder"
            )
...
class TestPersona(unittest.TestCase):
    ...
    def test_assess_breakthrough_evidence_publication_grade(self):
        evidence = self.aria.assess_breakthrough_evidence(
            metrics={
                "seeds_passed": 6,
                "total_seeds": 6,
...
class TestAnthropicBackendConfig(unittest.TestCase):
    ...
    def test_default_model_uses_alias(self):
        with patch.dict(os.environ, {}, clear=True):
            from research.scientist.llm.anthropic import (
                AnthropicBackend,
    ...
    def test_all_strategies_have_valid_keys(self):
        ...
        for i in range(9):  # 9 strategies
            ...
            for key in config:
                ...
                self.assertIn(key, valid_keys, f"Strategy {i} key '{key}' not valid")
