...
def test_low_rank_proj_equivalence():
    ...
    actual_flat = aria_core.linear_low_rank_f32(
        x_flat, U.contiguous(), V.contiguous(), None
    )
...
def test_bottleneck_proj_equivalence():
    ...
    x_flat = x.view(-1, D).contiguous()
    actual_flat = aria_core.linear_bottleneck_f32(
        x_flat, W_down.contiguous(), W_up.contiguous(), None, None
    )
    actual = actual_flat.view(B, S, D)
