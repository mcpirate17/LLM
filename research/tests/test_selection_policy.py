...
def _seed_candidates(nb: LabNotebook, exp_id: str):
    rows = [
        dict(graph_fingerprint="a1", graph_json=_graph_json("attention"), stage0_passed=1,
             stage05_passed=1, stage1_passed=1, loss_ratio=0.52, baseline_loss_ratio=0.86,
             novelty_score=0.33, throughput_tok_s=220.0, flops_per_token=1.3,
             peak_memory_mb=410.0, stability_score=0.80, has_nan_grad=0, has_zero_grad=0),
        dict(graph_fingerprint="b1", graph_json=_graph_json("conv1d_seq"), stage0_passed=1,
             stage05_passed=1, stage1_passed=1, loss_ratio=0.58, baseline_loss_ratio=0.90,
             novelty_score=0.77, throughput_tok_s=165.0, flops_per_token=1.1,
             peak_memory_mb=300.0, stability_score=0.92, has_nan_grad=0, has_zero_grad=0),
        dict(graph_fingerprint="c1", graph_json=_graph_json("gelu"), stage0_passed=1,
             stage05_passed=1, stage1_passed=0, loss_ratio=0.70, baseline_loss_ratio=0.96,
             novelty_score=0.60, throughput_tok_s=250.0, flops_per_token=0.9,
             peak_memory_mb=280.0, stability_score=0.60, has_nan_grad=0, has_zero_grad=1),
    ]
    out = []
    for item in rows:
...
def test_candidate_scoring_is_deterministic_for_fixed_db():
    ...
    exp_id = nb.start_experiment("synthesis", {"n_programs": 3}, "selection determinism")
...
def test_mode_selection_logs_decision_and_applies_safety_valve():
    ...
    runner.aria.recommend_next_mode = MagicMock(return_value={
        "mode": "synthesis",
        "reasoning": "default",
        "confidence": 0.4,
        "config": {},
...
def test_selection_insight_interactions_learn_supported_vs_not_supported():
    ...
    exp_id = nb.start_experiment("synthesis", {"n_programs": 2}, "insight interaction learning")
    i1 = nb.record_insight("success_factor", "Top op: attention", exp_id, confidence=0.9)
    i2 = nb.record_insight("pattern", "Pair: attention + conv1d_seq", exp_id, confidence=0.8)
...
def test_refinement_source_selection_is_deterministic_and_diverse():
    ...
    exp_id = nb.start_experiment("synthesis", {"n_programs": 8}, "deterministic refinement seeds")
    for idx in range(8):
        nb.record_program_result(
            exp_id,
...
def test_auto_recommend_records_next_experiment_plan_decision():
    ...
    exp_id = nb.start_experiment("synthesis", {"n_programs": 3}, "plan recommendation seed")
    for idx in range(3):
        nb.record_program_result(
            exp_id,
...
def test_recursive_refinement_runs_generation_and_records_decision():
    ...
    for idx in range(4):
        ...
        seed_ids.append(rid)
    nb.complete_experiment(seed_exp, {
        "total": 4,
        "stage0_passed": 4,
        "stage05_passed": 4,
        "stage1_passed": 4,
    ...
    def _fake_synthesis(cfg, fake_nb, n_exp, _limit, _reason):
        ...
        fake_nb.complete_experiment(exp, {
            "total": max(1, len(str(cfg.refine_source_result_ids or "").split(","))),
            "stage0_passed": 1,
            "stage05_passed": 1,
            "stage1_passed": 1,
            "best_loss_ratio": 0.45,
    ...
    with patch.object(runner, "_run_continuous_synthesis", side_effect=_fake_synthesis):
        runner._run_continuous_refinement(cfg, nb, n_experiments=2, limit_str="test", mode_reasoning="loop")
