...
class TestUnaryBackward:
    ...
    def test_gelu_backward(self):
        ...
        gelu_plus = (
            x_plus
            * 0.5
            * (1.0 + np.tanh(np.sqrt(2.0 / np.pi) * (x_plus + 0.044715 * x_plus**3)))
        )
        gelu_minus = (
            x_minus
            * 0.5
            * (1.0 + np.tanh(np.sqrt(2.0 / np.pi) * (x_minus + 0.044715 * x_minus**3)))
        )
    ...
    def test_shape_mismatch_raises(self):
        with pytest.raises(AssertionError):
            aria_bridge.dispatch_unary_backward(
                "relu", np.ones(10, dtype=np.float32), np.ones(20, dtype=np.float32)
            )
