...
class DummyAria(Aria):
    ...
    def summarize_graph(self, graph_json):
        return "Mock Summary"

    def generate_hypothesis(self, context):
        return "Mock Hypothesis"

    def analyze_interaction(self, results):
        return {"status": "ok"}
    ...
    def validate_breakthrough(self, result, hypothesis):
    ...
    def experiment_summary(self, results, context=""):
        return "Mock Summary"

    def analyze_results(self, results, context=""):
        return "Mock Analysis"

    def explain_fingerprint(self, context):
        return "Mock Fingerprint"

    def plan_strategy(self, context):
        return "Mock Strategy"

    def suggest_experiment(self, context="", **kwargs):
        return {"experiment_type": "synthesis", "config": {}}
...
def test_hydra_synthesis_to_leaderboard_e2e(temp_research_dir):
    ...
    try:
        ...
        progs = nb.conn.execute(
            "SELECT * FROM program_results WHERE experiment_id = ?", (exp_id,)
        ).fetchall()
        if len(progs) == 0:
            ...
            nb.flush_writes()
            progs = nb.conn.execute(
                "SELECT * FROM program_results WHERE result_id = ?", (seeded_result_id,)
            ).fetchall()
        assert len(progs) > 0
        prog = progs[0]
        res_id = prog["result_id"]
        sample_eff = (
            prog["sample_efficiency"] if "sample_efficiency" in prog.keys() else None
        )
        ...
        lb_entries = nb.conn.execute(
            "SELECT * FROM leaderboard WHERE result_id = ?", (res_id,)
        ).fetchall()
...
if __name__ == "__main__":
    ...
    try:
        test_hydra_synthesis_to_leaderboard_e2e(
            (tmp, os.path.join(tmp, "manual.db"), os.path.join(tmp, "manual.txt"))
        )
