...
def dead_parameterized_mask_natively(
    *, reachable_mask: np.ndarray, parameterized_flags: np.ndarray
    ...
    if not hasattr(lib, "aria_graph_dead_parameterized_mask"):
        raise RuntimeError("native dead-parameterized-mask symbol is unavailable")
    ...
    if status != 0:
        raise RuntimeError(
            f"aria_graph_dead_parameterized_mask failed with status={status}"
        )
...
def dead_parameterized_mask(
    *, reachable_mask: np.ndarray, parameterized_flags: np.ndarray
    ...
    return dead_parameterized_mask_natively(
