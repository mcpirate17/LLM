...
EXTENDED_MOTIFS: Tuple[Motif, ...] = (
    ...
    # ── New mixing primitives (2026-04-15) ─────────────────────────────
    Motif(
        name="attn_difficulty_routed",
        motif_class=MOTIF_CLASS_ATTENTION,
        steps=(
            MotifStep("difficulty_routed_attention", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Difficulty-routed attention: hard tokens full attention, easy tokens cheap linear → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=3.0,
    ),
    Motif(
        ...
        motif_class=MOTIF_CLASS_ATTENTION,
        steps=(
            MotifStep("strided_attention", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Strided multi-head attention (multi-scale strides per head) → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=2.5,
    ),
    Motif(
        name="attn_gated_progressive",
        motif_class=MOTIF_CLASS_ATTENTION,
        steps=(
            MotifStep("gated_progressive_attention", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Progressive gated attention (gate learns 0→1 engagement) → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=2.5,
    ),
    Motif(
        name="attn_gated_linear",
        motif_class=MOTIF_CLASS_ATTENTION,
        steps=(
            MotifStep("gated_linear_attention", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Gated Linear Attention (GLA) with data-dependent decay → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=3.0,
    ),
    Motif(
        name="mix_long_conv_hyena",
        motif_class=MOTIF_CLASS_SSM,
        steps=(
            MotifStep("long_conv_hyena", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Hyena implicit long convolution + multiplicative gating → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=3.0,
    ),
    Motif(
        name="attn_associative_memory",
        motif_class=MOTIF_CLASS_ATTENTION,
        steps=(
            MotifStep("associative_memory", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Modern Hopfield associative memory (learnable temperature) → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=3.0,
    ),
    ...
    Motif(
        ...
        motif_class=MOTIF_CLASS_SSM,
        steps=(
            MotifStep("mixture_of_recursions", OpRole.MIX),
            MotifStep("linear_proj", OpRole.PROJECT),
        ),
        description="Mixture-of-Recursions: adaptive depth per token via shared block → projection",
        support=0,
        avg_loss_ratio=0.0,
        lift=3.0,
    ),
