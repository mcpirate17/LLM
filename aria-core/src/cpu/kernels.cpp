...
void aria_tropical_center_f32(const float *x, float *y,
    ...
    for (int64_t b = 0; b < batch; b++) {
        for (int64_t d = 0; d < dim; d++) {
            float cumulative_min = INFINITY;
            for (int64_t s = 0; s < seq; s++) {
                int64_t idx = (b * seq + s) * dim + d;
                float v = x[idx];
                if (v < cumulative_min) cumulative_min = v;
                y[idx] = v - cumulative_min;
            }
        }
    }
}
...
void aria_layernorm_f32(const float *x, const float *weight, const float *bias,
    ...
    for (int64_t b = 0; b < batch; b++) {
        ...
        for (int64_t i = 0; i < vec_end; i += ARIA_SIMD_WIDTH) {
            v_sum = aria_simd_add_ps(v_sum, aria_simd_loadu_ps(xb + i));
        }
        float tmp[ARIA_SIMD_WIDTH];
        aria_simd_storeu_ps(tmp, v_sum);
        for (int i = 0; i < ARIA_SIMD_WIDTH; i++) sum += tmp[i];
        for (int64_t i = vec_end; i < dim; i++) sum += xb[i];
#else
        for (int64_t i = 0; i < dim; i++) sum += xb[i];
#endif
        ...
        for (int64_t i = 0; i < vec_end; i += ARIA_SIMD_WIDTH) {
            aria_simd_ps vx = aria_simd_loadu_ps(xb + i);
            aria_simd_ps vw = aria_simd_loadu_ps(weight + i);
            aria_simd_ps vb = aria_simd_loadu_ps(bias + i);
            aria_simd_ps normed = aria_simd_mul_ps(aria_simd_sub_ps(vx, v_mean), v_inv_std);
            aria_simd_ps vy = aria_simd_fmadd_ps(normed, vw, vb);
            aria_simd_storeu_ps(yb + i, vy);
        }
        for (int64_t i = vec_end; i < dim; i++) {
...
#if defined(ARIA_SIMD_WIDTH)
        int64_t vec_end = dim - (dim % ARIA_SIMD_WIDTH);
        aria_simd_ps v_ss = aria_simd_zero_ps;
        for (int64_t i = 0; i < vec_end; i += ARIA_SIMD_WIDTH) {
            aria_simd_ps vx = aria_simd_loadu_ps(xb + i);
            v_ss = aria_simd_fmadd_ps(vx, vx, v_ss);
        }
        float tmp[ARIA_SIMD_WIDTH];
        aria_simd_storeu_ps(tmp, v_ss);
        for (int i = 0; i < ARIA_SIMD_WIDTH; i++) ss += tmp[i];
        for (int64_t i = vec_end; i < dim; i++) ss += xb[i] * xb[i];
#else
        for (int64_t i = 0; i < dim; i++) {
            ss += xb[i] * xb[i];
        }
...
#if defined(ARIA_SIMD_WIDTH)
        aria_simd_ps v_inv_rms = aria_simd_set1_ps(inv_rms);
        for (int64_t i = 0; i < vec_end; i += ARIA_SIMD_WIDTH) {
            aria_simd_ps vx = aria_simd_loadu_ps(xb + i);
            aria_simd_ps vw = aria_simd_loadu_ps(weight + i);
            aria_simd_ps vy = aria_simd_mul_ps(vx, aria_simd_mul_ps(v_inv_rms, vw));
            aria_simd_storeu_ps(yb + i, vy);
        }
        for (int64_t i = vec_end; i < dim; i++) {
