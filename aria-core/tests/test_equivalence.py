...
class TestUnaryOps:
    ...
    def test_relu(self):
        torch.testing.assert_close(aria_core.relu_f32(self.x), F.relu(self.x), atol=ATOL, rtol=RTOL)

    def test_gelu(self):
        # C kernel uses tanh approximation
        expected = 0.5 * self.x * (1 + torch.tanh(math.sqrt(2/math.pi) * (self.x + 0.044715 * self.x**3)))
        torch.testing.assert_close(aria_core.gelu_f32(self.x), expected, atol=1e-4, rtol=1e-3)

    def test_silu(self):
        torch.testing.assert_close(aria_core.silu_f32(self.x), F.silu(self.x), atol=1e-4, rtol=1e-3)

    def test_sigmoid(self):
        torch.testing.assert_close(aria_core.sigmoid_f32(self.x), torch.sigmoid(self.x), atol=1e-4, rtol=1e-3)

    def test_tanh(self):
        torch.testing.assert_close(aria_core.tanh_f32(self.x), torch.tanh(self.x), atol=1e-4, rtol=1e-3)

    def test_exp(self):
        x = self.x.clamp(-10, 10)
        torch.testing.assert_close(aria_core.exp_f32(x), torch.exp(x), atol=1e-4, rtol=1e-3)

    def test_square(self):
        torch.testing.assert_close(aria_core.square_f32(self.x), self.x ** 2, atol=ATOL, rtol=RTOL)

    def test_abs(self):
        torch.testing.assert_close(aria_core.abs_f32(self.x), self.x.abs(), atol=ATOL, rtol=RTOL)

    def test_neg(self):
        torch.testing.assert_close(aria_core.neg_f32(self.x), -self.x, atol=ATOL, rtol=RTOL)

    def test_sin(self):
        torch.testing.assert_close(aria_core.sin_f32(self.x), torch.sin(self.x), atol=1e-4, rtol=1e-3)

    def test_cos(self):
        torch.testing.assert_close(aria_core.cos_f32(self.x), torch.cos(self.x), atol=1e-4, rtol=1e-3)

    def test_log(self):
        x = self.x.abs() + 0.01
        torch.testing.assert_close(aria_core.log_f32(x), torch.log(x), atol=1e-4, rtol=1e-3)

    def test_sqrt(self):
        x = self.x.abs() + 0.01
        torch.testing.assert_close(aria_core.sqrt_f32(x), torch.sqrt(x), atol=1e-4, rtol=1e-3)

    def test_reciprocal(self):
        x = self.x.abs() + 0.1
        torch.testing.assert_close(aria_core.reciprocal_f32(x), 1.0 / x, atol=1e-4, rtol=1e-3)
...
class TestBinaryOps:
    ...
    def test_add(self):
        torch.testing.assert_close(aria_core.add_f32(self.a, self.b), self.a + self.b, atol=ATOL, rtol=RTOL)

    def test_mul(self):
        torch.testing.assert_close(aria_core.mul_f32(self.a, self.b), self.a * self.b, atol=ATOL, rtol=RTOL)

    def test_sub(self):
        torch.testing.assert_close(aria_core.sub_f32(self.a, self.b), self.a - self.b, atol=ATOL, rtol=RTOL)

    def test_maximum(self):
        torch.testing.assert_close(aria_core.maximum_f32(self.a, self.b), torch.maximum(self.a, self.b), atol=ATOL, rtol=RTOL)

    def test_minimum(self):
        torch.testing.assert_close(aria_core.minimum_f32(self.a, self.b), torch.minimum(self.a, self.b), atol=ATOL, rtol=RTOL)

    def test_tropical_add(self):
        # tropical add = elementwise min
        torch.testing.assert_close(aria_core.tropical_add_f32(self.a, self.b), torch.minimum(self.a, self.b), atol=ATOL, rtol=RTOL)

    def test_div_safe(self):
        b = self.b.abs() + 0.1
        torch.testing.assert_close(aria_core.div_safe_f32(self.a, b), self.a / b, atol=1e-4, rtol=1e-3)
...
class TestLinAlg:
    def test_matmul(self):
        A = torch.randn(8, 16)
        B = torch.randn(16, 4)
        torch.testing.assert_close(aria_core.matmul_f32(A, B), A @ B, atol=1e-3, rtol=1e-3)

    def test_linear_with_bias(self):
        x = torch.randn(4, 8)
        W = torch.randn(16, 8)
        b = torch.randn(16)
        torch.testing.assert_close(aria_core.linear_f32(x, W, b), F.linear(x, W, b), atol=1e-3, rtol=1e-3)

    def test_linear_no_bias(self):
        x = torch.randn(4, 8)
        W = torch.randn(16, 8)
        torch.testing.assert_close(aria_core.linear_f32(x, W, None), F.linear(x, W), atol=1e-3, rtol=1e-3)
...
class TestSoftmax:
    def test_softmax(self):
        x = torch.randn(4, 16)
        torch.testing.assert_close(aria_core.softmax_f32(x), F.softmax(x, dim=-1), atol=1e-4, rtol=1e-3)
...
class TestFused:
    def test_matmul_relu(self):
        A = torch.randn(4, 8)
        B = torch.randn(8, 16)
        torch.testing.assert_close(aria_core.matmul_relu_f32(A, B), F.relu(A @ B), atol=1e-3, rtol=1e-3)
    ...
    def test_matmul_gelu(self):
        ...
        mm = A @ B
        expected = 0.5 * mm * (1 + torch.tanh(math.sqrt(2/math.pi) * (mm + 0.044715 * mm**3)))
...
class TestBackward:
    def test_relu_backward(self):
        ...
        torch.testing.assert_close(aria_core.relu_backward_f32(go, x), expected, atol=ATOL, rtol=RTOL)
...
class TestTropical:
    def test_tropical_matmul(self):
        ...
        for i in range(4):
            for j in range(3):
                for k in range(8):
                    expected[i, j] = min(expected[i, j].item(), A[i, k].item() + B[k, j].item())
        torch.testing.assert_close(result, expected, atol=1e-4, rtol=1e-3)
