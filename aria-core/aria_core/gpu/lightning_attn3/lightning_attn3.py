...
# Decay variant with exponential decay factors
# SPDX-License-Identifier: Apache-2.0

import torch
import triton

# Cache for backward tile sizes per device (autotune-like selection)
_BWD_TILE_CACHE: dict[int, tuple[int, int]] = {}
...
class LightningAttention3(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, do):
        ...
        # --- Blackwell-safe backward tiling with autotune-like selection ---
        # SM 12.0 consumer GPUs (RTX 50xx) have ~101KB shared mem limit
        # We cache the best config per device to avoid repeated probing
        device_idx = q.device.index
        if device_idx not in _BWD_TILE_CACHE:
            props = torch.cuda.get_device_properties(q.device)
            cc_major = props.major
            if cc_major >= 12:
                # Blackwell: use 32/16 (power-of-2 required by Triton)
                # 32/16 uses ~43KB shared mem, fits in 101KB limit
                _BWD_TILE_CACHE[device_idx] = (32, 16)  # Blackwell-safe (power of 2)
            else:
                # Pre-Blackwell / datacenter: use fastest config
                _BWD_TILE_CACHE[device_idx] = (64, 32)
        
        BLOCK, CBLOCK = _BWD_TILE_CACHE[device_idx]
...
lightning_attn3 = LightningAttention3.apply
