        ...
        for steps in steps_grid:
            for mode in modes:
                ...
                try:
                    ...
                    res = {"status": f"exc: {e}", "auc": 0.0, "gap_acc": {},
                ...
                row = {
                    ...
                    "n_train_steps": steps, "train_mode": mode,
                    "auc": res["auc"],
                ...
                w.writerow(row)
                f.flush()
                print(f"  {arch:<12} steps={steps:4} {mode:<6} "
                      f"auc={res['auc']:.3f} "
...
def resume_induction_extended():
    ...
    target_archs = [
        "hybrid_2l", "ssm_4l", "rwkv_2l", "conv7_4l",  # missed when main crashed
    ]
    ...
    for arch in target_archs:
        ...
        for steps in steps_grid:
            ...
            row = {
                "arch": arch, "n_params": n_params,
                "n_train_steps": steps, "train_mode": "mixed",
                "auc": res["auc"],
                "max_gap_acc": res.get("max_gap_acc", 0.0),
                "min_gap_acc": res.get("min_gap_acc", 0.0),
                "acc_4": gap_acc.get(4, 0.0),
            ...
            w.writerow(row)
            f.flush()
            print(f"  {arch:<12} steps={steps:4} auc={res['auc']:.3f} "
                  f"peak={res.get('max_gap_acc', 0):.3f} "
                  f"time={res['elapsed_s']}s")
        del base
        if DEVICE == "cuda":
    ...
    for arch in archs:
        ...
        for steps in steps_grid:
            ...
            try:
                ...
                res = {"status": f"exc: {e}", "auc": 0.0, "dist_acc": {},
                       "first_loss": 0.0, "last_loss": 0.0, "elapsed_s": 0.0}
            dist_acc = res.get("dist_acc", {})
            row = {
                "arch": arch, "n_params": n_params, "n_train_steps": steps,
                "auc": res["auc"],
    ...
    for arch in archs:
        ...
        for steps in steps_grid:
            ...
            try:
                ...
                res = {"status": f"exc: {e}", "auc": 0.0, "final_acc": 0.0,
                       "first_loss": 0.0, "last_loss": 0.0, "elapsed_s": 0.0}
            row = {
                "arch": arch, "n_params": n_params, "n_train_steps": steps,
                "auc": res["auc"], "final_acc": res["final_acc"],
                "first_loss": res.get("first_loss", 0.0),
                "last_loss": res.get("last_loss", 0.0),
                "status": res["status"],
                "elapsed_s": res["elapsed_s"],
...
if __name__ == "__main__":
    ...
    for step in (resume_induction, resume_induction_extended,
                 resume_binding_curriculum, resume_ar):
        try:
            step()
        except Exception as e:
